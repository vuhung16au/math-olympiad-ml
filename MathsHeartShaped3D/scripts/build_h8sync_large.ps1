# Build Script for H8sync Effect (Large Resolution)
# Automates the complete workflow: audio analysis -> video generation -> audio combination

param(
    [string]$AudioFile = "inputs/H8InfiniteStars2_100s.mp3",
...
Write-Section "H8sync Effect Builder - Large Resolution"
...
Write-Section "Step 2: Generating Heart Animation with Audio Sync"

$outputVideo = "outputs/h8sync_${Resolution}.mp4"

Write-Info "Generating video with H8sync effect..."
Write-Host "  Output: $outputVideo" -ForegroundColor White
Write-Host "  Effect: H8sync (Real Audio Sync)" -ForegroundColor White
Write-Host "  Resolution: $Resolution" -ForegroundColor White
Write-Host "  Density: $Density" -ForegroundColor White
Write-Host "  FPS: $FPS" -ForegroundColor White
Write-Host "  Bitrate: $Bitrate kbps" -ForegroundColor White
...
# Build command
$videoCommand = "& `"$pythonCmd`" heart_animation.py --effect H8sync --audio-features $featuresFile --resolution $Resolution --density $Density --fps $FPS --bitrate $Bitrate --output $outputVideo"
...
$finalOutput = "outputs/h8sync_${Resolution}_with_audio.mp4"
...
# Get video duration for audio trimming (100 seconds)
$audioCommand = "ffmpeg -i $outputVideo -i $AudioFile -t 100 -c:v copy -c:a aac -b:a 192k -y $finalOutput"
...
Write-Success "H8sync animation with audio synchronization is ready!"
Write-Host ""
...
Write-Host "  • Effect: H8sync (Heart Genesis with Real Audio Sync)" -ForegroundColor White
Write-Host ""
