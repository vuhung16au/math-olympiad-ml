# Build Script for H9 Effect: "Cuba to New Orleans" - A Musical Journey Through the Heart
# Automates the complete workflow: audio analysis -> video generation -> audio combination

param(
    [string]$AudioFile = "inputs/Cuba.....NewOrleans.mp3",
...
Write-Section "H9 Effect Builder: Cuba to New Orleans - Musical Journey Through the Heart"
...
Write-Host "Duration: ~698 seconds (11:38)" -ForegroundColor White
Write-Host ""
...
Write-Section "Step 2: Generating Heart Animation with H9 Effect"

$outputVideo = "outputs/h9_cuba_neworleans_4k.mp4"

Write-Info "Generating video with H9 effect..."
Write-Host "  Output: $outputVideo" -ForegroundColor White
Write-Host "  Effect: H9 (Cuba to New Orleans - Musical Journey Through the Heart)" -ForegroundColor White
Write-Host "  Resolution: $Resolution" -ForegroundColor White
Write-Host "  Density: $Density" -ForegroundColor White
Write-Host "  FPS: $FPS" -ForegroundColor White
Write-Host "  Bitrate: $Bitrate kbps" -ForegroundColor White
...
Write-Host ""
Write-Host "  WARNING: This will take a VERY long time (1-3 hours) for 4K rendering!" -ForegroundColor Red
Write-Host "  Consider using lower resolution for testing (e.g., -Resolution large)" -ForegroundColor Yellow
Write-Host ""

$videoStartTime = Get-Date

# Build command
$videoCommand = "& `"$pythonCmd`" heart_animation.py --effect H9 --audio-features $featuresFile --resolution $Resolution --density $Density --fps $FPS --bitrate $Bitrate --output $outputVideo"
...
$finalOutput = "outputs/h9_cuba_neworleans_4k_with_audio.mp4"

Write-Info "Combining $outputVideo with $AudioFile..."
Write-Host "  Output: $finalOutput" -ForegroundColor White
Write-Host "  Audio duration: ~698 seconds (11:38)" -ForegroundColor White
Write-Host ""

$audioStartTime = Get-Date

# Get actual audio duration
$audioDuration = & $pythonCmd -c "import librosa; y, sr = librosa.load('$AudioFile'); print(len(y) / sr)"

# Combine video with full audio
$audioCommand = "ffmpeg -i $outputVideo -i $AudioFile -t $audioDuration -c:v copy -c:a aac -b:a 192k -shortest -y $finalOutput"
...
Write-Success "H9 animation with audio synchronization is ready!"
Write-Host ""
