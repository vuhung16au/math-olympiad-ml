...
\begin{takeaways}
\item For a union, add the two probabilities and subtract the overlap once.
\item The overlap here is the king of diamonds, which belongs to both events.
\end{takeaways}
...
\begin{takeaways}
\item In a repeated yes-no experiment, count the successful patterns before multiplying by the common probability.
\item This is a small binomial-style question with $n=3$ and $p=\frac12$.
\end{takeaways}
...
\begin{takeaways}
\item Without replacement, the second probability changes because the box contents change.
\item Multiplying stage-by-stage is often the quickest method for short draws.
\end{takeaways}
...
\begin{takeaways}
\item Expected value is a weighted average, not just an ordinary mean of the outcomes.
\item Multiply each value by its probability, then add.
\end{takeaways}
...
\begin{takeaways}
\item Turn the box counts into probabilities before finding the expected cost.
\item In modelling questions, keep the distinction clear between an item's cost and its selection probability.
\end{takeaways}
...
\begin{takeaways}
\item When a distribution table is given, answer event questions by adding the probabilities of all matching outcomes.
\item Read the condition carefully: ``odd'' and ``at least $2$'' select different sets of $x$-values.
\end{takeaways}
...
\begin{takeaways}
\item ``Not'' often signals a complement.
\item If the complement is easier to count, find it first and subtract from $1$.
\end{takeaways}
...
\begin{takeaways}
\item In conditional probability, first reduce the sample space to the given condition.
\item If only one outcome remains possible, the conditional probability is $1$.
\end{takeaways}
...
\begin{takeaways}
\item ``Exactly one'' in repeated trials usually leads to a binomial coefficient.
\item The coefficient counts where the single success can occur.
\end{takeaways}
...
\begin{takeaways}
\item For independent devices, multiply the separate probabilities.
\item A coin-and-die question is a classic independence model.
\end{takeaways}
...
\begin{takeaways}
\item Complements are especially efficient in one-draw questions.
\item Here ``not red'' is the same event as drawing blue.
\end{takeaways}
...
\begin{takeaways}
\item In equally likely letter questions, each position in the word counts, including repeated letters.
\item Be careful not to switch from counting letters to counting distinct symbols.
\end{takeaways}
...
\begin{takeaways}
\item A Bernoulli random variable takes only the values $0$ and $1$.
\item Its mean and variance are controlled entirely by the success probability $p$.
\end{takeaways}
