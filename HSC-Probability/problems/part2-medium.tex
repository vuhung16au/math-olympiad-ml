...
\begin{takeaways}
\item Conditional probability starts by shrinking the sample space to the given condition.
\item After conditioning on ``red'', only the red cards matter.
\end{takeaways}
...
\begin{takeaways}
\item To test independence, compare $P(A\cap B)$ with $P(A)P(B)$.
\item Similar-looking events are not automatically independent.
\end{takeaways}
...
\begin{takeaways}
\item A probability distribution must add to $1$ before any expectation calculation is valid.
\item Solve for missing probabilities first, then use the weighted-average formula.
\end{takeaways}
...
\begin{takeaways}
\item Once a binomial model is identified, mean and variance come straight from the standard formulas.
\item Always check that both $n$ and $p$ have been read correctly from the question.
\end{takeaways}
...
\begin{takeaways}
\item For a union, add the two groups and subtract the overlap once.
\item Counts can be handled first, then converted to probability at the end.
\end{takeaways}
...
\begin{takeaways}
\item Variance from a table usually needs both $E(X)$ and $E(X^2)$.
\item Keep the square on the value of $X$, not on the probability.
\end{takeaways}
...
\begin{takeaways}
\item This is a without-replacement counting problem, so combinations are the natural tool.
\item A full distribution table should list all possible values of $X$ and probabilities that add to $1$.
\end{takeaways}
...
\begin{takeaways}
\item A binomial distribution table comes from choosing where the heads occur.
\item The expected value matches the shortcut formula $E(X)=np$.
\end{takeaways}
...
\begin{takeaways}
\item Hypergeometric questions model sampling without replacement.
\item This contrasts with binomial models, where trials are independent and replacement is implicit.
\end{takeaways}
...
\begin{takeaways}
\item ``At least'' questions are often quickest by using the complement.
\item For binomial probabilities, check whether adding a few terms or subtracting a few terms is shorter.
\end{takeaways}
...
\begin{takeaways}
\item ``Exactly one of each type'' is a strong sign to use combinations.
\item Without replacement, the order does not matter when you count selections with combinations.
\end{takeaways}
...
\begin{takeaways}
\item For binomial counts, the mean is the number of trials times the success probability.
\item The expected number of tails need not be an integer outcome.
\end{takeaways}
...
\begin{takeaways}
\item Symmetry is a powerful shortcut for $\mathrm{Bin}\!\left(n,\frac12\right)$.
\item Odd and even $n$ behave differently because only even $n$ has a central term at $X=\frac n2$.
\end{takeaways}
...
\begin{takeaways}
\item Translate the real situation into a binomial count before calculating.
\item Here the known answers shift the target: passing means at least $4$ correct guesses, not $9$ guessed questions.
\end{takeaways}
...
\begin{takeaways}
\item ``Same metal'' splits naturally into two disjoint cases: both silver or both bronze.
\item When cases cannot happen together, their probabilities can be added directly.
\end{takeaways}
...
\begin{takeaways}
\item Mixed-device questions may need more than one model in the same expression.
\item Independence lets you multiply the treadmill event by the rowing-machine event.
\end{takeaways}
...
\begin{takeaways}
\item ``At least one'' is often easiest through the complement ``none''.
\item A large success probability over many trials pushes $P(X\ge 1)$ very close to $1$.
\end{takeaways}
