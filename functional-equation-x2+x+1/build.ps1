# PowerShell build script for functional-equation-x2+x+1.tex
...
$TEXFILE = "functional-equation-x2+x+1"
$PDFFILE = "$TEXFILE.pdf"
...
function Build-PDF {
    ...
    $localPdflatex = Get-Command pdflatex -ErrorAction SilentlyContinue
    
    if ($localPdflatex) {
        Write-Host "Using local pdflatex installation..." -ForegroundColor Cyan
        ...
        Write-Host "Using Docker for LaTeX compilation..." -ForegroundColor Cyan
        ...
        if ($LASTEXITCODE -ne 0) {
            Write-Host "First compilation pass failed!" -ForegroundColor Red
            Write-Host "Make sure Docker is running or install a local LaTeX distribution." -ForegroundColor Yellow
            exit 1
        }
