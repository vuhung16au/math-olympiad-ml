...
\vspace{0.5cm}
\rotatebox{180}{\parbox{\textwidth}{\small \textbf{Hint:} The theorem expresses $(x+a)^n$ as a sum involving binomial coefficients $\binom{n}{r} = \frac{n!}{r!(n-r)!}$.}}
...
\vspace{0.5cm}
\rotatebox{180}{\parbox{\textwidth}{\small \textbf{Hint:} These are elementary symmetric polynomials. Construct cubic $P(x) = x^3 - x^2 + 9x - 9$ with roots $p, q, r$. Factor by grouping.}}
...
\vspace{0.5cm}
\rotatebox{180}{\parbox{\textwidth}{\small \textbf{Hint:} Use quadratic formula with $a=i$. Find $\sqrt{3-4i}$ by setting $\sqrt{3-4i}=x+iy$ and solving. Part (b): Use Vieta's formula $\alpha\beta = -1/i = i$.}}
...
\vspace{0.5cm}
\rotatebox{180}{\parbox{\textwidth}{\small \textbf{Hint:} The product equals 4. For distinct integer factors, the only way is $\{1, 2, -1, -2\}$ with product 4. Their sum is 0.}}
...
\vspace{0.5cm}
\rotatebox{180}{\parbox{\textwidth}{\small \textbf{Hint:} If $x = p/q$ (coprime), then $p^3 + pq^2 + q^3 = 0$. Check all parity cases: (odd,odd), (odd,even), (even,odd). All lead to odd=even contradiction.}}
