...
\item In an HSC proof question, full marks usually come from showing the key substitutions, the cancellation, and the final conclusion clearly, not just writing the final answer.
\item Writing the process matters: markers need to see how part (i) is applied in part (ii), so short linking phrases such as ``using part (i)'' and ``this is a telescoping sum'' are worth including.
\end{takeaways}

\begin{remark}[Marking guide note]
For a question like this, students should read the marking guide as a list of \emph{visible ingredients} their solution must contain. In part (i), a correct rearrangement with the substitution $n=m+1$ and $r=R+1$ is enough. In part (ii), full marks usually require more than the final identity: show how part (i) is applied to the general term, write out enough of the expanded sum to make the telescoping pattern visible, and then state why the endpoint terms simplify. In short, a neat mathematical answer is good, but an examiner awards marks for a clear mathematical \emph{process}.
\end{remark}
...
\begin{problem}[Australian Powerball counting]
...
\begin{enumerate}[label=\roman*)]
\item How many different standard Powerball game lines are possible?
\item Hence show that the probability of winning Division~1 (all $7$ regular numbers and the Powerball) is
\[
\frac{1}{134490400}.
\]
\item For a fixed winning draw, in how many different game lines would a player match exactly $3$ regular numbers and the Powerball? Hence show that this probability is about $1$ in $188$.
\end{enumerate}
\end{problem}

\begin{solution}
\textbf{i)} Choose $7$ regular numbers from $35$, then choose $1$ Powerball from $20$:
\[
\binom{35}{7}\times 20=6724520\times 20=134490400.
\]
So there are $\boxed{134490400}$ standard game lines.

\textbf{ii)} For a fixed draw, exactly one line wins Division~1, so
\[
P(\text{Division 1})=\frac{1}{134490400}.
\]

\textbf{iii)} To match exactly $3$ regular numbers, choose $3$ of the $7$ winning regular numbers and $4$ of the $28$ non-winning regular numbers. The Powerball must also be correct:
\[
...
\begin{takeaways}
\item Lottery questions often split naturally into ``choose the regular numbers'' and ``choose the Powerball''.
\item ``Exactly $k$ matches'' means choose some winning numbers and the rest from the non-winning numbers.
\item Published lottery odds are often rounded, so a combinatorial answer may be an approximation rather than a perfect exact fraction.
\end{takeaways}
...
\item Find the probability that the plate says \texttt{12AB34}.
\end{enumerate}
\end{problem}
...
\textbf{c)} There is exactly one favourable plate, namely \texttt{12AB34}, out of all possible license plates. Therefore
\[
\frac{1}{6760000}.
\]
\[
...
\item If a question asks for one exact plate, the probability is usually $\frac{1}{\text{total number of plates}}$.
