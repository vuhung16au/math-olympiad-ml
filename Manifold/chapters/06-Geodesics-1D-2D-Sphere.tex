...
A fundamental principle is that geodesics minimize path length. Mathematically, if we have a path $\gamma(t)$ between two points $P_1$ and $P_2$ on a manifold, its length is given by (see Definition~\ref{def:length-functional}):
...
\noindent\textit{Notes.} $\dot{x}^i = \tfrac{dx^i}{dt}$ denotes time derivatives of the coordinates along the curve $\gamma(t)$; $L$ is the Lagrangian (here derived from length/energy); $t$ is a curve parameter; $x^i$ are local coordinates on the manifold.

\begin{remark}[Euler–Lagrange refresher]
...
\begin{remark}[Antipodal points]
Two points on a circle or sphere are \emph{antipodal} if they are diametrically opposite (separated by half the circumference on the circle or by a central angle of $\pi$ on the sphere). On a sphere, every great circle through one antipodal point also passes through the other.
\end{remark}
...
The set of points where geodesics cease to be unique is called the \textbf{cut locus}. 
\begin{example}[Cut locus examples]
% \begin{example}[Arc-length parameterization examples]
For example:
\begin{itemize}
...
\begin{example}[Straight line in different coordinates]
...
\begin{example}[Great circle on a sphere]
...
A manifold is \textbf{geodesically complete} if every geodesic can be extended indefinitely (in both directions).

\begin{example}[Geodesic completeness examples]
...
\begin{example}[Circle geodesic via variational method]
...
\begin{example}[Sphere geodesic via geometric construction]
