...
\begin{remark}[Singularity and singular point]
In differential geometry, a \emph{singularity} is a point where the smooth structure or geometric quantities fail to be well-defined (e.g., not differentiable) or become unbounded. A \emph{singular point} of a surface is a point at which no coordinate chart yields a smooth embedding with a regular (full-rank) differential, so standard objects like a well-defined tangent plane or curvature tensors cannot be assigned in the usual way. For a cone, the tip is singular because any local parametrization has rank deficiency at the apex, and curvature measures are not defined there.
\end{remark}

\begin{remark}[Curvature (brief)]
Curvature quantifies deviation from flatness. On surfaces in $\mathbb{R}^3$, \emph{Gaussian curvature} $K$ at a regular point is the product of principal curvatures and can be computed from the first and second fundamental forms; for smooth Riemannian manifolds, curvature is encoded by the Riemann curvature tensor $R$, with sectional curvature giving a 2D-plane slice of $R$. Smooth points have finite, well-defined curvature; at the cone tip, curvature is not defined in the smooth sense (it concentrates as a singular defect).
\end{remark}
...
\begin{example}[Other non-manifold cases]
