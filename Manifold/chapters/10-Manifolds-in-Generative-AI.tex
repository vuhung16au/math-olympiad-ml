...
\begin{example}[Text generation (e.g., ChatGPT)]
Viewing model parameters as a \emph{parameter manifold} with the Fisher metric yields \emph{natural gradient} updates that follow geodesics in information space, often converging faster than Euclidean gradients. Inference traverses a low-dimensional \emph{data manifold} of plausible token sequences, where curvature helps explain why small Euclidean steps can produce large semantic shifts and why geometry-aware sampling (e.g., temperature, nucleus) stabilizes outputs.
\end{example}

\begin{example}[Image generation (diffusion, GANs, flows)]
Diffusion models learn score fields that align with the manifold of natural images; denoising follows integral curves tangent to this manifold. Normalizing flows learn \emph{diffeomorphisms} between a simple latent manifold and image space; the pullback metric clarifies conditioning and mode coverage. In GANs, geodesic distances in latent space better preserve semantics than Euclidean pixel metrics, guiding interpolation and editing.
\end{example}

\begin{example}[Video generation]
...
A \emph{parameter manifold} is a differentiable manifold $\Theta$ of model parameters endowed with a metric (e.g., the Fisher metric or a pullback metric), which induces lengths/angles of parameter displacements and defines geometry-aware optimization.
\end{definition}
...
\caption{The data manifold hypothesis: high-dimensional data points (like images) 
lie on a low-dimensional manifold $\mathcal{M}$ embedded in the ambient space. 
Points off the manifold are typically invalid or meaningless.}
\label{fig:data-manifold}

\end{figure}


\begin{remark}[Ambient Space]
The ambient space is the high-dimensional space in which the data lies. For example, 
the ambient space for an image is $\mathbb{R}^{256 \times 256 \times 3}$.
\end{remark}


\begin{remark}[Latent Space]
The latent space is the low-dimensional space in which the data is represented. For example, 
the latent space for an image is $\mathbb{R}^{50-200}$.
\end{remark}
...
\item \textbf{Video}: $D = 256 \times 256 \times 3 \times 30$ (256x256x3 for each frame, 30 frames), but $d \approx 50-200$ for natural videos
\end{itemize}
...
\subsection{Core properties (see also Fisher Information Matrix)}
\begin{itemize}
...
\subsection{Worked examples (see also Natural Gradient, Parameter Manifolds)}
\begin{itemize}
...
\subsection{Practical estimation and approximations (see also Natural Gradient)}
\begin{itemize}
...
\subsection{Examples and case studies (see also Fisher, Bregman Divergence)}
\begin{itemize}
...
\subsection{Practical guidance (see also Natural Gradient)}
...
Given a Riemannian metric $\mathbf{G}(\boldsymbol{\theta})$ on parameter space, the \emph{natural gradient} of $\mathcal{L}$ is $\tilde\nabla_{\boldsymbol{\theta}}\mathcal{L}=\mathbf{G}(\boldsymbol{\theta})^{-1}\nabla_{\boldsymbol{\theta}}\mathcal{L}$. With $\mathbf{G}=\mathbf{F}$, this yields reparameterization-invariant updates.
\end{definition}
...
\subsection{Usage guidance and caveats (see also Fisher, Parameter Manifolds)}
...
For strictly convex differentiable $\varphi$, the \emph{Bregman divergence} is $D_{\varphi}(p\,\|\,q)=\varphi(p)-\varphi(q)-\langle\nabla\varphi(q),p-q\rangle$. It is generally asymmetric and not a metric.
\end{definition}
...
\subsection{Optimization links (see also Natural Gradient, Parameter Manifolds)}
...
\subsection{Practical guidance (see also Fisher, Natural Gradient)}
