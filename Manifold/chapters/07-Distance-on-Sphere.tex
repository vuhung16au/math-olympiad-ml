...
\begin{definition}[Haversine function]
The \emph{haversine} of an angle $\theta$ is
\[
...
\begin{remark}[Distance formulas summary]
\textbf{Basic (arccos)}: $\; d = R\,\arccos\!\big(\sin\phi_1\sin\phi_2 + \cos\phi_1\cos\phi_2\cos\Delta\lambda\big)$.\\
\textbf{Haversine}: $\; d = 2R\,\arcsin\!\Big(\sqrt{\sin^2(\tfrac{\Delta\phi}{2}) + \cos\phi_1\cos\phi_2\,\sin^2(\tfrac{\Delta\lambda}{2})}\;\Big)$.\\
\textbf{Spherical law of cosines (central angle)}: $\; \cos\tfrac{d}{R} = \sin\phi_1\sin\phi_2 + \cos\phi_1\cos\phi_2\cos\Delta\lambda$.
\end{remark}
...
\begin{example}[Same meridian: numerical example]
...
\begin{example}[Same parallel (latitude)]
...
\begin{example}[Same parallel: numerical example]
...
\begin{example}[Near poles: numerical considerations]
...
\begin{example}[Near-pole numerical example]
...
\begin{example}[Flat Earth approximation accuracy]
...
\begin{example}[arccos precision issue]
...
\begin{example}[Multiple cities route]
...
\begin{example}[Comparing formulas]
...
\begin{example}[Near-polar distance]
