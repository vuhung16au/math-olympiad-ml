...
A \emph{topological $n$-manifold} is a second-countable, Hausdorff topological space $\mathcal{M}$ such that every point $p\in\mathcal{M}$ has an open neighborhood $U$ and a homeomorphism (chart) $\varphi:U\to V\subset\mathbb{R}^n$. An \emph{atlas} is a collection of charts whose domains cover $\mathcal{M}$. If all transition maps $\varphi_j\circ\varphi_i^{-1}$ between overlapping charts are $C^k$ (typically $k=\infty$), $\mathcal{M}$ is a \emph{$C^k$ (smooth) manifold}.
\end{definition}

\begin{definition}[Chart and Atlas]
A \emph{chart} on $\mathcal{M}$ is a pair $(U,\varphi)$ with $U\subset\mathcal{M}$ open and $\varphi:U\to\varphi(U)\subset\mathbb{R}^n$ a homeomorphism. An \emph{atlas} is a family of charts whose domains cover $\mathcal{M}$; it is \emph{smooth} when all chart transitions are smooth.
\end{definition}
