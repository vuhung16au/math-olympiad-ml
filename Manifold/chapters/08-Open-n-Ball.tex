...
\begin{definition}[Open $n$-ball]
An \emph{open $n$-ball} in $\mathbb{R}^n$ of radius $r>0$ centered at $\mathbf{p}\in\mathbb{R}^n$ is
\[
B_r(\mathbf{p}) = \{\mathbf{x} \in \mathbb{R}^n : \|\mathbf{x} - \mathbf{p}\| < r\},
\]
where $\|\mathbf{x}-\mathbf{p}\|$ is the Euclidean norm. In coordinates, for $\mathbf{p}=(p_1,\dots,p_n)$ and $\mathbf{x}=(x_1,\dots,x_n)$,
\[
B_r(\mathbf{p}) = \Big\{(x_1,\dots,x_n)\in\mathbb{R}^n : \sum_{i=1}^n (x_i-p_i)^2 < r^2\Big\}.
\]
...
\begin{remark}[A mapping thought experiment]
Can you construct a one-to-one, continuous, onto map with continuous inverse from a 2D open disk $B_r(\mathbf{p})\subset\mathbb{R}^2$ to an open ball in $\mathbb{R}^n$ for $n\neq 2$? \;\emph{Hint}: Dimension is a topological invariant; by invariance of domain, such a homeomorphism cannot exist unless the dimensions match.
\end{remark}
...
\begin{example}[Distance in the unit disk]
...
\begin{example}[Distance in the unit ball]
...
\begin{definition}[Diameter via supremum]
...
\]
Here $\sup$ (\emph{supremum}) is the least upper bound of a set: the smallest number that is greater than or equal to every element in the set, not necessarily achieved by any element.
\end{definition}
...
A set is \emph{connected} if it cannot be written as the disjoint union of two nonempty open sets in the subspace topology. Equivalently, any two points can be joined by a continuous path lying in the set. An open ball is connected.
\end{definition}

\begin{remark}[Convex vs. concave]
A set is \emph{convex} if for any two points in it, the entire line segment between them lies in the set. “Concave” typically describes functions or shapes that curve inward; in set terms we usually contrast convex with nonconvex rather than “concave.”
\end{remark}
...
\begin{example}[Is a point in an open ball?]
...
\begin{example}[Distance between two points in a ball]
...
\begin{example}[Largest contained ball]
