...
\makeatother
\newcommand{\HSCBookletHeaderTitle}{The Nature of Proof}
...
This booklet presents a comprehensive collection of proof problems for HSC Mathematics Extension~2 students. The 38 carefully selected problems cover all essential proof techniques: Direct Proof, Proof by Contradiction, Mathematical Induction, and Proof by Cases. Topics include divisibility, irrationality, modular arithmetic, parity, and logical reasoning.

The collection is divided into two parts:
\begin{itemize}[leftmargin=*]
  \item \textbf{Part 1 (21 problems):} Detailed step-by-step solutions showing complete reasoning, algebraic manipulation, and justification of each step. Each solution is designed to fit within one page, modeling clear and concise proof writing.
  \item \textbf{Part 2 (18 problems):} Strategic hints and solution sketches using flipped hint environments, encouraging independent problem-solving while providing guidance on key steps.
\end{itemize}
...
\subsection{The Language of Logic: Reading Quantifier Statements}
...
\begin{itemize}
  \item $\forall$ --- ``for all'' / ``for every'' / ``for each'' (the \textit{universal} quantifier)
  \item $\exists$ --- ``there exists'' / ``there is at least one'' (the \textit{existential} quantifier)
\end{itemize}

The examples below show both a \textit{literal, symbol-by-symbol} translation and a \textit{natural English} paraphrase. Being able to move fluently between these two registers is essential for understanding what a statement actually claims before you attempt to prove (or disprove) it.

\medskip

\noindent\textbf{Statement 1} \quad $\forall x \in \mathbb{R},\ x^2 \ge 0$

\begin{itemize}
  \item \textit{Literal:} For all $x$ in the real numbers, $x$ squared is greater than or equal to zero.
  \item \textit{Natural English:} The square of any real number is always non-negative.
\end{itemize}

\medskip

\noindent\textbf{Statement 2} \quad $\forall n \in \mathbb{Z},\ \exists m \in \mathbb{Z},\ m = n + 5$

\begin{itemize}
  \item \textit{Literal:} For every integer $n$, there exists an integer $m$ such that $m$ equals $n$ plus $5$.
  \item \textit{Natural English:} Given any integer, you can always find another integer that is exactly $5$ greater than it. \emph{(True.)}
\end{itemize}

\medskip

\noindent\textbf{Statement 3} \quad $\exists a \in \mathbb{R},\ \forall x \in \mathbb{R},\ ax = x$

\begin{itemize}
  \item \textit{Literal:} There exists a real number $a$ such that for all real numbers $x$, $a$ times $x$ equals $x$.
  \item \textit{Natural English:} There is a specific real number that, when multiplied by any real number, leaves that number unchanged. \emph{(True; that number is $a = 1$, the multiplicative identity.)}
\end{itemize}

\medskip

\noindent\textbf{Statement 4} \quad $\exists m \in \mathbb{Z},\ \forall n \in \mathbb{Z},\ m = n + 5$

\begin{itemize}
  \item \textit{Literal:} There exists an integer $m$ such that for every integer $n$, $m$ equals $n$ plus $5$.
  \item \textit{Natural English:} There is a single ``magic'' integer that is simultaneously exactly $5$ greater than \emph{every} other integer. \emph{(False.)}
\end{itemize}

\bigskip

\noindent\textbf{Key Takeaway: Order of quantifiers matters.}

Statements~2 and~4 use \emph{identical symbols} arranged in a slightly different order, yet they have opposite truth values. This is one of the most common traps in the language of proof:

\begin{itemize}
  \item \textbf{Statement~2} ($\forall$ first, then $\exists$): the choice of $m$ is allowed to \emph{depend} on $n$. You name an $n$, and only then do you pick a matching $m$. This makes the statement true.
  \item \textbf{Statement~4} ($\exists$ first, then $\forall$): the $m$ must be chosen \emph{before} $n$ is known. It claims one fixed constant $m$ works for every single $n$ simultaneously --- which is impossible.
\end{itemize}

\begin{tcolorbox}[colback=blue!5!white, colframe=blue!60!black, title=Quantifier Rule of Thumb]
Swapping the order of $\forall$ and $\exists$ completely changes the mathematical meaning of a sentence. Always read quantifiers left-to-right and ask: \emph{who gets to choose first?}
\end{tcolorbox}
...
\input{problems/part1-01}
\newpage

\input{problems/part1-02}
\newpage

\input{problems/part1-03}
\newpage

\input{problems/part1-03a}
\newpage

\input{problems/part1-04}
\newpage

\input{problems/part1-05}
\newpage

\subsection*{Medium Problems (5 problems)}

\input{problems/part1-06}
\newpage

\input{problems/part1-07}
\newpage

\input{problems/part1-08}
\newpage

\input{problems/part1-09}
\newpage

\input{problems/part1-10}
\newpage

\subsection*{Hard Problems (10 problems)}

\input{problems/part1-11}
\newpage

\input{problems/part1-12}
\newpage

\input{problems/part1-12a}
\newpage

\input{problems/part1-13}
\newpage

\input{problems/part1-14}
\newpage

\input{problems/part1-15}
\newpage

\input{problems/part1-15a}
\newpage

\input{problems/part1-15b}
\newpage

\input{problems/part1-16}
\newpage

\input{problems/part1-17}
\newpage

\section{Part 2: Problems with Hints and Solutions}

\textit{The following problems provide strategic hints followed by worked solutions. Attempt each problem independently before consulting the hint, then use the solution to verify your argument structure and mathematical details.}

\vspace{1cm}

\subsection*{Easy Problems (5 problems)}

\input{problems/part2-01}
\newpage

\input{problems/part2-02}
\newpage

\input{problems/part2-03}
\newpage

\input{problems/part2-04}
\newpage

\input{problems/part2-05}
\newpage

\subsection*{Medium Problems (6 problems)}

\input{problems/part2-06}
\newpage

\input{problems/part2-07}
\newpage

\input{problems/part2-08}
\newpage

\input{problems/part2-09}
\newpage

\input{problems/part2-10}
\newpage

\input{problems/part2-21}
\newpage

\subsection*{Hard Problems (10 problems)}

\input{problems/part2-11}
\newpage

\input{problems/part2-12}
\newpage

\input{problems/part2-13}
\newpage

\input{problems/part2-14}
\newpage

\input{problems/part2-15}

\newpage

\input{problems/part2-16}

\newpage

\input{problems/part2-18}

\newpage

\input{problems/part2-19}

\newpage

\input{problems/part2-20}

\newpage

\input{problems/part2-22}

\newpage

\subsection*{Additional Problem}

\input{problems/part2-17}
...
Formal logic is now part of the HSC Extension~2 syllabus. \textbf{De Morgan's Laws} tell you how to push a negation ($\neg$) through brackets when a statement uses \textbf{AND} ($\land$) or \textbf{OR} ($\lor$). Get this wrong and a proof by contradiction starts from the wrong assumption.

\medskip
\noindent\textbf{The laws.} For statements $P$ and $Q$:
\begin{align*}
  \neg(P \land Q) &\equiv \neg P \lor \neg Q \\
  \neg(P \lor Q)  &\equiv \neg P \land \neg Q
\end{align*}
\noindent\textbf{Memory trick:} distribute $\neg$ inside the brackets and \emph{flip} $\land$ to $\lor$ (or $\lor$ to $\land$).

\medskip
\noindent\textbf{Negation of an implication} (often tested with contradiction):
\[
  \neg(P \implies Q) \equiv P \land \neg Q
\]
The negation of ``if $P$ then $Q$'' is \emph{not} another if-then. It is: $P$ happens, but $Q$ fails.

\medskip
\noindent\textbf{Example 1 (negating OR).} ``$a \leq 2$ or $b \leq 2$'' has negation ``$a > 2$ \textbf{and} $b > 2$''---Law~2 with $P$: $a \leq 2$, $Q$: $b \leq 2$. This is exactly the opening move in Part~1, Problem~8.

\medskip
\noindent\textbf{Example 2 (contradiction).} Prove: if $x^2$ is even, then $x$ is even ($P \implies Q$).

Assume, for contradiction, the \emph{exact} negation:
\[
  x^2 \text{ is even} \quad \land \quad x \text{ is odd}
\]
...
\medskip
\noindent\textbf{Why this matters.} Rigorous proof is not only algebra; it is \emph{precise logic}. Contradiction, contrapositive, and quantified statements all depend on correct negation. University mathematics (analysis, algebra, computer science) treats these equivalences as standard tools---not optional extras. Mastering De Morgan's Laws now prevents silent logical errors later.
...
Repository: \href{https://github.com/vuhung16au/math-olympiad-ml/tree/main/HSC-Proofs}{https://github.com/vuhung16au/math-olympiad-ml/tree/main/HSC-Proofs}
