...
\begin{tikzpicture}[scale=1.5,>=Stealth]
  ...
  % Nudge the label to avoid overlapping the adjacent polygon edges.
  \node[above,font=\scriptsize] at ($(P1)+(0.12,0)$) {$P_1$};
...
Let $\triangle ABC$ be an equilateral triangle with vertices 
$A$, $B$, $C$ represented by complex numbers 
$a$, $b$, $c$ respectively, oriented anticlockwise. 
...
\begin{problem}[Principal Square Root and Argument Locus]

For a real number $x > 0$, the square root is denoted as $ \sqrt{x} $ that $ \sqrt{x} \cdot \sqrt{x} = x $ and it is unique. 

We can extend this definition to complex numbers as follows, 
but koala in mind that the square root of a complex number is not unique.

\textbf{Definition.} Every non-zero complex number $w = re^{i\theta}$ has two square roots. If $\theta$ is the principal argument
(that is, $-\pi < \theta \le \pi$) of $w$, the \textbf{principal square root} $\sqrt{w}$ is
\[
\sqrt{w} = \sqrt{r}\,e^{i\theta/2}.
\]
Let $z$ be a non-zero complex number with principal argument $-\pi < \arg(z) \le \pi$.

\begin{enumerate}[label=(\alph*),leftmargin=*]
  \item Using the definition above, show that
  \[
  \sqrt{-z} =
  \begin{cases}
    -i\sqrt{z}, & 0 < \arg(z) \le \pi,\\[4pt]
    i\sqrt{z}, & -\pi < \arg(z) \le 0.
  \end{cases}
  \]
  \item Hence, solve $\sqrt{z} + \sqrt{-z} = 2$ for $z \in \mathbb{C}$.
  \item Let $z_1$ and $z_2$ be the solutions from part~(b) with $\text{Im}(z_1) > 0$. A point $P$ represents $w$ on the Argand diagram. The locus of $P$ satisfies
  \[
...
\begin{hint}
\begin{itemize}[leftmargin=*]
  \item \textbf{(a)} Write $z = re^{i\theta}$. Multiplying by $-1$ rotates by $\pi$; adjust $\arg(-z)$ into $(-\pi,\pi]$ by adding or subtracting $\pi$ according to the quadrant of $\theta$.
  \item \textbf{(b)} Split into the two cases from part~(a). Solve for $\sqrt{z}$ first, square to obtain $z$, then verify $\arg(z)$ lies in the case you used.
  \item \textbf{(c)} Substitute $w = x + iy$ and $z_1 = 2i$, $z_2 = -2i$. Rationalise the denominator. For argument $\pi/3$, require positive real and imaginary parts, then use $\tan(\pi/3) = \sqrt{3}$.
\end{itemize}
\end{hint}

\begin{solution}[Full Solution]
\textbf{Part (a).} Let $z = re^{i\theta}$ with $r > 0$ and $-\pi < \theta \le \pi$, so $\sqrt{z} = \sqrt{r}\,e^{i\theta/2}$.

Multiplying by $-1$ rotates $z$ by $\pi$. To keep the principal argument in $(-\pi,\pi]$:
\begin{itemize}[leftmargin=*]
  \item If $0 < \theta \le \pi$, then $\arg(-z) = \theta - \pi$ and
  \[
  \sqrt{-z} = \sqrt{r}\,e^{i(\theta-\pi)/2}
  = \sqrt{r}\,e^{i\theta/2}e^{-i\pi/2} = -i\sqrt{z}.
  \]
  \item If $-\pi < \theta \le 0$, then $\arg(-z) = \theta + \pi$ and
  \[
  \sqrt{-z} = \sqrt{r}\,e^{i(\theta+\pi)/2}
  = \sqrt{r}\,e^{i\theta/2}e^{i\pi/2} = i\sqrt{z}.
  \]
...
\textbf{Part (b).}
\begin{itemize}[leftmargin=*]
  \item \textbf{Case 1} ($0 < \arg(z) \le \pi$): $\sqrt{-z} = -i\sqrt{z}$ gives $\sqrt{z}(1-i)=2$, so $\sqrt{z} = \dfrac{2}{1-i} = 1+i$ and $z = (1+i)^2 = 2i$. Check: $\arg(2i)=\pi/2$.
  \item \textbf{Case 2} ($-\pi < \arg(z) \le 0$): $\sqrt{-z} = i\sqrt{z}$ gives $\sqrt{z}(1+i)=2$, so $\sqrt{z} = 1-i$ and $z = (1-i)^2 = -2i$. Check: $\arg(-2i)=-\pi/2$.
\end{itemize}
Hence $z = \pm 2i$.

\textbf{Part (c).} Take $z_1 = 2i$, $z_2 = -2i$, and $w = x + iy$. Then
\[
\frac{w-z_1}{w-z_2}
= \frac{x + i(y-2)}{x + i(y+2)}
= \frac{x^2 + y^2 - 4 - 4ix}{x^2 + (y+2)^2}.
\]
For $\arg(\cdot) = \pi/3$ (first quadrant), need $x^2 + y^2 - 4 > 0$ and $-4x > 0$, so $x < 0$. Also
\[
\sqrt{3} = \frac{-4x}{x^2 + y^2 - 4}
\quad\Longrightarrow\quad
x^2 + \frac{4\sqrt{3}}{3}x + y^2 = 4.
\]
Completing the square in $x$:
\[
\left(x + \frac{2\sqrt{3}}{3}\right)^2 + y^2 = \frac{16}{3}.
\]
The locus is the major arc of the circle with centre $\left(-\dfrac{2\sqrt{3}}{3},\,0\right)$ and radius $\dfrac{4\sqrt{3}}{3}$, lying in the left half-plane ($x < 0$), with endpoints $(0,2)$ and $(0,-2)$ excluded.
...
\begin{tikzpicture}[scale=1.35,>=Stealth]
  ...
  \draw[->,thick] (-3.8,0) -- (1.2,0) node[right,font=\small] {Re};
  \draw[->,thick] (0,-2.8) -- (0,2.8) node[above,font=\small] {Im};

  \draw[acu circle,dashed,warmstone!60] (C) circle ({\R});
  \draw[very thick,bookpurple] (C) ++ (60:{\R}) arc (60:300:{\R});

  \fill[acu point] (C) circle (1.4pt);
  \node[below,font=\scriptsize] at (C) {centre};

  \coordinate (Zone) at (0,2);
  \coordinate (Ztwo) at (0,-2);
  \fill[lawpurple] (Zone) circle (1.3pt);
  \fill[lawpurple] (Ztwo) circle (1.3pt);
  \node[right,font=\scriptsize] at (Zone) {$z_1=2i$};
  \node[right,font=\scriptsize] at (Ztwo) {$z_2=-2i$};
  \draw[bookred,thick] (Zone) circle (2.4pt);
  \draw[bookred,thick] (Ztwo) circle (2.4pt);
  \node[left,font=\tiny,bookred] at (0,1.55) {excluded};

  \draw[dashed,bookpurple!40] (C) -- (Zone);
  \node[font=\scriptsize,bookpurple] at ($ (C)!0.55!(Zone) + (0.22,0) $) {$R=\tfrac{4\sqrt{3}}{3}$};

  \node[font=\scriptsize,align=left,draw=purple!50,fill=white,rounded corners] at (-2.6,2.2) {
    Circle: $\left(x+\tfrac{2\sqrt{3}}{3}\right)^2+y^2=\tfrac{16}{3}$\\
    Locus: major arc with $x<0$\\
    Endpoints $(0,\pm2)$ excluded
  };
\end{tikzpicture}
\end{center}
\end{solution}
...
\begin{problem}[Bounding Complex Magnitude]
Let $z$ be a complex number such that $|z| = 1$. Let $z = \cos\theta + i\sin\theta$.

\begin{enumerate}[label=(\roman*),leftmargin=*]
    \item Show that $|z^3 - z + 2|^2 = 6 - 2\cos2\theta + 4\cos3\theta - 4\cos\theta$.
    \item Letting $x = \cos\theta$, show that $|z^3 - z + 2|^2$ can be expressed as the polynomial $P(x)$, where $P(x) = 16x^3 - 4x^2 - 16x + 8$. \\
    \textit{(You are given that $\cos3\theta = 4\cos^3\theta - 3\cos\theta$)}
    \item Find the critical values of the polynomial $P(x)$ in the domain $-1 \le x \le 1$.
    \item Hence, or otherwise, determine the maximum value of $|z^3 - z + 2|$.
\end{enumerate}
\end{problem}

\begin{hint}
\begin{itemize}[leftmargin=*]
    \item \textbf{(i)} Use De Moivre's Theorem to express $z^3$, separate into real and imaginary parts, and apply $|w|^2 = \text{Re}(w)^2 + \text{Im}(w)^2$. The compound angle formula $\cos(A-B)$ cleans up the cross-terms.
    \item \textbf{(ii)} Substitute the given identity for $\cos3\theta$, and use $\cos2\theta = 2x^2 - 1$.
    \item \textbf{(iii)} Differentiate $P(x)$, set $P'(x) = 0$, and factor the quadratic to find turning points.
    \item \textbf{(iv)} Test critical values and the domain endpoints ($x = 1, -1$) in $P(x)$. Remember to take the square root of the maximum value.
\end{itemize}
\end{hint}

\begin{solution}[Sketch]
\textbf{(i)} $z^3 = \cos3\theta + i\sin3\theta$. \\
$z^3 - z + 2 = (\cos3\theta - \cos\theta + 2) + i(\sin3\theta - \sin\theta)$. \\
$|z^3 - z + 2|^2 = (\cos3\theta - \cos\theta + 2)^2 + (\sin3\theta - \sin\theta)^2$ \\
$= (\cos^23\theta + \sin^23\theta) + (\cos^2\theta + \sin^2\theta) + 4 - 2(\cos3\theta\cos\theta + \sin3\theta\sin\theta) + 4\cos3\theta - 4\cos\theta$ \\
$= 6 - 2\cos(3\theta - \theta) + 4\cos3\theta - 4\cos\theta$ \\
$= 6 - 2\cos2\theta + 4\cos3\theta - 4\cos\theta$

\textbf{(ii)} Substitute $\cos2\theta = 2x^2 - 1$ and $\cos3\theta = 4x^3 - 3x$: \\
$P(x) = 6 - 2(2x^2 - 1) + 4(4x^3 - 3x) - 4x$ \\
$P(x) = 6 - 4x^2 + 2 + 16x^3 - 12x - 4x$ \\
$P(x) = 16x^3 - 4x^2 - 16x + 8$

\textbf{(iii)} $P'(x) = 48x^2 - 8x - 16$. \\
Set $P'(x) = 0 \implies 6x^2 - x - 2 = 0 \implies (3x - 2)(2x + 1) = 0$. \\
Critical values: $x = \frac{2}{3}, -\frac{1}{2}$, both in $[-1, 1]$.

\textbf{(iv)} Test endpoints and critical values: \\
$P(1) = 4$, $P(-1) = 4$, $P\left(\frac{2}{3}\right) = \frac{8}{27}$, $P\left(-\frac{1}{2}\right) = 13$. \\
Maximum $|z^3 - z + 2|^2 = 13$. Maximum value of $|z^3 - z + 2|$ is $\sqrt{13}$.
\end{solution}

\begin{takeaways}
\begin{itemize}[leftmargin=*]
    \item \textbf{Bridging Domains:} Converting a complex magnitude problem into a real-variable calculus problem is a powerful technique for bounding.
    \item \textbf{Endpoint Checks:} Reinforces that students must check domain endpoints for global maximums.
\end{itemize}
\end{takeaways}

\begin{problem}[Inequality from Complex Number Manipulation]
Let $z$ be a non-zero complex number such that $|z^3 + z^{-3}| \leq 2$.
By letting $w = z + z^{-1}$, 

\textbf{(i)} Show that $z^3 + z^{-3} = w^3 - 3w$. \\
\textbf{(ii)} Hence, using triangle inequality, or otherwise, prove that $|z + z^{-1}| \leq 2$.
\end{problem}

\begin{hint}
\textbf{Part (i):} Start by expanding $(z + z^{-1})^3$ using the binomial theorem and group the resulting terms to isolate $z^3 + z^{-3}$. \\
\textbf{Part (ii):} Recall the reverse triangle inequality: $|a - b| \ge |a| - |b|$. Apply this to $|w^3 - 3w|$ and proceed with a proof by contradiction assuming $|w| > 2$.
\end{hint}
...
\begin{align*}
w^3 &= z^3 + z^{-3} + 3w \\
\therefore z^3 + z^{-3} &= w^3 - 3w
\end{align*}

\textbf{(ii)} We are given $|z^3 + z^{-3}| \leq 2$.
Substituting the result from (i):
\[ |w^3 - 3w| \leq 2 \]
Using the reverse triangle inequality $|a - b| \ge |a| - |b|$:
\begin{align*}
|w^3 - 3w| &\ge |w^3| - |3w| \\
|w^3 - 3w| &\ge |w|^3 - 3|w| \\
\implies |w|^3 - 3|w| &\leq |w^3 - 3w| \leq 2
\end{align*}
Assume for the sake of contradiction that $|w| > 2$.
Let $f(x) = x^3 - 3x$, where $x = |w|$. We are analyzing $f(x)$ for $x > 2$.
\[ f'(x) = 3x^2 - 3 \]
For $x > 2$, $f'(x) > 0$, so the function is strictly increasing.
Therefore, if $x > 2$, $f(x) > f(2)$.
\[ f(2) = 2^3 - 3(2) = 8 - 6 = 2 \]
Thus, if $|w| > 2$, it must be true that $|w|^3 - 3|w| > 2$.
However, we have already established that $|w|^3 - 3|w| \leq 2$.
This directly contradicts our finding.
Therefore, our assumption must be false, meaning $|w| \leq 2$.
\[ \therefore |z + z^{-1}| \leq 2 \]
\end{solution}

\begin{takeaways}
\begin{itemize}[leftmargin=*]
    \item \textbf{Scaffolding complex proofs:} Defining a strategic intermediate variable ($w = z + z^{-1}$) is a key technique to unlock complex algebraic relationships.
    \item \textbf{Reverse Triangle Inequality:} Applying $|a - b| \ge |a| - |b|$ in non-standard contexts is a frequent differentiator in high-level HSC exams.
    \item \textbf{Logical rigor:} Constructing a formal proof by contradiction by combining algebraic manipulation with function analysis (evaluating bounds using derivatives) is essential for solving advanced inequalities.
\end{itemize}
\end{takeaways}
