...
\begin{problem}[The Spira Mirabilis - Eadem mutata resurgo]
% \section*{Problem Statement}

\begin{quote}
  	\textit{"Eadem mutata resurgo"} --- "Changed, I rise again the same." \\
...
\begin{enumerate}
  \item[(i)] Prove that for all $n \ge 0$, the triangle formed by the origin $O$ and two consecutive terms, $\triangle O z_n z_{n+1}$, is similar to the triangle $\triangle O z_{n+1} z_{n+2}$.
    
  \item[(ii)] Find the length of the polygonal spiral path connecting the points $z_0, z_1, z_2, \dots$:
  \[ L = \sum_{n=0}^{\infty} |z_{n+1} - z_n| \]
    
  \item[(iii)] Show that the points $z_n$ lie on the continuous logarithmic spiral defined by the polar equation:
  \[ r = e^{-k\theta} \]
  and find the exact value of the constant $k$.
    
  \item[(iv)] Let $R = |z_0|$. Prove that the ratio of the total path length to the initial radius is given by:
...
\begin{tikzpicture}[scale=3]
  \draw[->] (-0.5,0) -- (1.2,0) node[right] {Re};
  \draw[->] (0,-0.5) -- (0,1) node[above] {Im};
    
  \coordinate (O) at (0,0);
  \coordinate (Z0) at (1,0);
  \coordinate (Z1) at (0.5, 0.5);
  \coordinate (Z2) at (0, 0.5);
  \coordinate (Z3) at (-0.25, 0.25);
  \coordinate (Z4) at (-0.25, 0);
  % Additional points along the discrete spiral (z_n = ((1+i)/2)^n)
  \coordinate (Z5) at (-0.125, -0.125);
  \coordinate (Z6) at (0, -0.125);
  \coordinate (Z7) at (0.0625, -0.0625);
  \coordinate (Z8) at (0.0625, 0);
  \coordinate (Z9) at (0.03125, 0.03125);
  \coordinate (Z10) at (0, 0.03125);
    
  \draw[thick, blue] (Z0) -- (Z1) -- (Z2) -- (Z3) -- (Z4) -- (Z5) -- (Z6) -- (Z7) -- (Z8) -- (Z9) -- (Z10);
  \draw[dashed, gray] (O) -- (Z0);
  \draw[dashed, gray] (O) -- (Z1);
  \draw[dashed, gray] (O) -- (Z2);
  \draw[dashed, gray] (O) -- (Z3);
  \draw[dashed, gray] (O) -- (Z4);
  \draw[dashed, gray] (O) -- (Z5);
  \draw[dashed, gray] (O) -- (Z6);
    
  \fill[blue] (Z0) circle (0.02) node[below right] {$z_0$};
  \fill[blue] (Z1) circle (0.02) node[above right] {$z_1$};
  \fill[blue] (Z2) circle (0.02) node[above left] {$z_2$};
  \fill[blue] (Z3) circle (0.02) node[above left] {$z_3$};
  \fill[blue] (Z4) circle (0.02) node[below left] {$z_4$};
  \fill[blue] (Z5) circle (0.02) node[below left] {$z_5$};
  \fill[blue] (Z6) circle (0.02) node[below] {$z_6$};
    
  % Approximate spiral curve
  \draw[red, domain=0:4, samples=100, smooth] plot ({exp(-0.44*\x)*cos(\x r)}, {exp(-0.44*\x)*sin(\x r)});
  \node[red] at (0.2, 0.1) {Spiral};
\end{tikzpicture}
\end{center}
...
\begin{hint}
\begin{itemize}
  \item \textbf{Part (i):} Use the properties of complex multiplication. If $z_{n+1} = w z_n$, then multiplication by $w$ scales the modulus by $|w|$ and rotates the argument by $\arg(w)$. Compare the ratios of sides and included angles for the two triangles.
  \item \textbf{Part (ii):} The sequence of segment lengths forms a geometric progression. First, find $|z_{n+1} - z_n|$ in terms of $|z_n|$ and the constant multiplier.
  \item \textbf{Part (iii):} Express the multiplier in exponential form $r e^{i\phi}$. This allows you to write $z_n$ as a function of $n$. Eliminate $n$ to relate the modulus (radius) directly to the argument (angle).
  \item \textbf{Part (iv):} Calculate $\cot(\pi/8)$ exactly using half-angle formulae and compare it to your result from Part (ii).
\end{itemize}
\end{hint}

\begin{solution}[Detailed]
Let $w=\frac{1+i}{2}$. Then $|w|=\frac{|1+i|}{2}=\frac{\sqrt{2}}{2}=\frac{1}{\sqrt{2}}$ and $\arg(w)=\frac{\pi}{4}$.

	\textbf{(i) Self-similarity.}
Since $z_{n+1}=w\,z_n$, multiplication by $w$ maps the vector $Oz_n$ to $Oz_{n+1}$ by scaling all lengths by $|w|$ and rotating all directions by $\arg(w)$. Hence both the included angle $\angle z_nOz_{n+1}$ and the ratio $\frac{|Oz_{n+1}|}{|Oz_n|}$ are constant and equal to $\arg(w)$ and $|w|$, respectively. By SAS, $\triangle Oz_nz_{n+1}\sim\triangle Oz_{n+1}z_{n+2}$.

	\textbf{(ii) Total polygonal length.}
The $n$-th segment length is
\[
...
\[
L=\sum_{n=0}^{\infty}\ell_n=\sum_{n=0}^{\infty}\left(\frac{1}{\sqrt{2}}\right)^{n+1}
=\frac{\frac{1}{\sqrt{2}}}{1-\frac{1}{\sqrt{2}}}
=\frac{1}{\sqrt{2}-1}
=\sqrt{2}+1.
\]

	\textbf{(iii) Continuous spiral form.}
Write $w=\frac{1}{\sqrt{2}}e^{i\pi/4}$. Then
\[
...
\[
r=2^{-\frac{1}{2}\cdot\frac{4\theta}{\pi}}=2^{-\frac{2\theta}{\pi}}
=e^{-\frac{2\ln 2}{\pi}\,\theta},
\]
hence $r=e^{-k\theta}$ with $k=\frac{2\ln 2}{\pi}$.

	\textbf{(iv) Ratio as a trig value.}
With $R=|z_0|=1$, we have $\frac{L}{R}=\sqrt{2}+1$. To link this to $\cot\!\left(\frac{\pi}{8}\right)$, set $t=\tan\!\left(\frac{\pi}{8}\right)$ and use the double-angle identity
\[
\tan(2A)=\frac{2\tan A}{1-\tan^2 A},\quad A=\frac{\pi}{8},
\]
together with $\tan\!\left(\frac{\pi}{4}\right)=1$. Then
\[
1=\frac{2t}{1-t^2}\ \Longrightarrow\ t^2+2t-1=0
\ \Longrightarrow\ t=\sqrt{2}-1 \quad(\text{positive since } \tfrac{\pi}{8}\in(0,\tfrac{\pi}{4})).
\]
Thus
\[
...
%   \item Multiplication by a complex constant produces a fixed rotation and scaling. Here, $z_{n+1} = wz_n$ with $w=\frac{1+i}{2}$ rotates by $\arg(w)=\frac{\pi}{4}$ and scales by $|w|=\frac{1}{\sqrt{2}}$.
%   \item The spiral path length is an infinite geometric series. Although the path has infinitely many segments, the total length is finite.
%   \item The sequence $\{z_n\}$ gives a discrete model of a logarithmic spiral, connecting algebraic recurrence with a continuous polar equation.
%   \item The logarithmic spiral is self-similar and appears in natural and applied contexts, including shells, galaxies, and antenna geometry.
% \end{itemize}
...
A point $P$ represents the complex number 
$z = 3 + 2i$ on the Argand diagram. Mark the point $R$ representing $ R = iz$.
\end{problem}
...
Points $P$ and $Q$ represent the complex numbers 
$z = 2 + i$ and $w = 1 + 3i$ respectively on the Argand diagram. 
Mark point $T$ representing $ T = z+w$.
\end{problem}
...
\begin{tikzpicture}[scale=1.4,>=Stealth]
  ...
  % Parallelogram Law legend (right of diagram, clear of vectors and labels)
  \node[
    anchor=west,
    font=\scriptsize,
    align=left,
...
\begin{tikzpicture}[scale=1.3,>=Stealth]
  ...
  \draw[->,very thick,bookpurple] (0,0) -- (Z);
  \fill[bookpurple] (Z) circle (2pt);
  \node[bookpurple,font=\small,anchor=west] at ($(Z)+(0.12,0.08)$) {$z$};
  
  % Label for z (right of the point, clear of the vector and circle)
  \node[bookpurple,font=\scriptsize,align=left,anchor=west] at ($(Z)+(0.65,0.05)$) {
    $|z|=2$\\
    $\arg(z)=\frac{\pi}{4}$
  };
  ...
  \node[bookred,font=\small,anchor=east] at ($(U)+(-0.2,0.05)$) {$u=z^2$};
  
  % Label for u (above the point, clear of the vector and coordinate)
  \node[bookred,font=\scriptsize,align=left,anchor=south west] at ($(U)+(0.15,0.45)$) {
    $|u|=4$\\
    $\arg(u)=\frac{\pi}{2}$
  };
  ...
  \node[bookpurple,font=\tiny,anchor=north] at ($(Z)+(0.05,-0.25)$) {$(\sqrt{2},\sqrt{2})$};
  \node[bookred,font=\tiny,anchor=west] at ($(U)+(0.35,-0.2)$) {$(0,4)$};
  
  % Origin
  \fill (0,0) circle (1pt);
  \node[below left,font=\scriptsize] at (0,0) {$O$};
  
  % Squaring Rule legend (right of diagram, clear of vectors and arcs)
  \node[
    anchor=west,
    font=\scriptsize,
    align=left,
  ...
  % Modulus labels on the circles (left/right, away from vectors)
  \node[bookpurple!60,font=\tiny] at (160:1.55) {$r=2$};
  \node[bookred!60,font=\tiny] at (120:3.55) {$r=4$};
...
\begin{problem}[Region with Modulus and Argument Constraints]
Sketch the region in the Argand plane defined by the set:
$$S = \left\{ z \in \mathbb{C} : 1 \le z\bar{z} \le 9 \right\} \cap \left\{ z \in \mathbb{C} : \frac{\pi}{4} \le \arg(z) \le \frac{3\pi}{4} \right\}$$
\textit{(Use the polar grid scaffold below to draw your solution).}
...
\begin{center}
    \begin{tikzpicture}[scale=1.5]
        ...
        \draw[->, thick] (0,-3.5) -- (0,3.5) node[above] {$\text{Im}(z)$};

        % Ticks and Labels
        \foreach \x in {-3, -2, -1, 1, 2, 3} {
            \draw[thick] (\x, 0.05) -- (\x, -0.05) node[below right] {$\x$};
        }
        \foreach \y in {-3, -2, -1, 1, 2, 3} {
            \draw[thick] (0.05, \y) -- (-0.05, \y) node[above left] {$\y$};
        }
...
\begin{hint}
\begin{itemize}[leftmargin=*]
    \item \textbf{Hint 1:} Recall the fundamental identity $z\bar{z} = |z|^2$. What geometric shape does the boundary $|z|^2 = r^2$ represent?
    \item \textbf{Hint 2:} The inequality $1 \le |z|^2 \le 9$ forms an annulus (a ring). Determine its inner and outer radii.
    \item \textbf{Hint 3:} The condition $\frac{\pi}{4} \le \arg(z) \le \frac{3\pi}{4}$ restricts the region to a specific sector.
    \item \textbf{Hint 4:} Pay attention to the inequality symbols ($\le$). Boundary lines should be solid.
\end{itemize}
\end{hint}

\begin{solution}[Sketch]
\textbf{1. Analyze the Modulus Condition:}\\
Using $z\bar{z} = |z|^2$, the first condition becomes:
$$1 \le |z|^2 \le 9 \implies 1 \le |z| \le 3$$
Geometrically, this represents the closed annulus lying between the circles of radius 1 and 3 centered at the origin.

\textbf{2. Analyze the Argument Condition:}\\
The condition $\frac{\pi}{4} \le \arg(z) \le \frac{3\pi}{4}$ describes a sector swept between the ray at $45^\circ$ ($\frac{\pi}{4}$) and the ray at $135^\circ$ ($\frac{3\pi}{4}$).

\textbf{3. Combine and Sketch:}\\
The set $S$ is the intersection ($\cap$) of these two conditions. We shade the portion of the annulus trapped between the two rays. Because all inequalities are inclusive ($\le$), all boundary curves are solid.
...
\begin{center}
    \begin{tikzpicture}[scale=1.5]
        % Scaffold (Background)
        \foreach \angle in {45, 135, 225, 315} {
            \draw[gray!50, thin] (0,0) -- (\angle:3.2);
        }
        \draw[gray!50, thin] (0,0) circle (1);
        \draw[gray!50, thin] (0,0) circle (2);
        \draw[gray!50, thin] (0,0) circle (3);

        % Shaded Region
        \fill[blue, opacity=0.2] (45:1) arc (45:135:1) -- (135:3) arc (135:45:3) -- cycle;
        
        % Boundary Lines (Solid because of <=)
        \draw[blue, thick] (45:1) arc (45:135:1);
        \draw[blue, thick] (135:3) arc (135:45:3);
        \draw[blue, thick] (45:1) -- (45:3);
        \draw[blue, thick] (135:1) -- (135:3);
...
% \textbf{Takeaways:}
% \begin{itemize}[leftmargin=*]
%     \item \textbf{The Modulus-Conjugate Bridge:} The identity $z\bar{z} = |z|^2$ is an essential tool.
%     \item \textbf{Polar Thinking:} When a problem involves $z\bar{z}$ and $\arg(z)$, thinking in polar coordinates $(r, \theta)$ makes the sketch significantly easier.
%     \item \textbf{Intersection of Loci:} When dealing with sets intersecting ($\cap$), sketch boundaries lightly first, then boldly outline and shade the overlapping region.
% \end{itemize}
\end{solution}
