...
\begin{problem}[Problem 45: Cauchy-Schwarz and Sums]
...
So $\sum_{k=1}^n \frac{S}{S-x_k} \geq \frac{n^2}{n-1}$.
\end{solution}
...
\begin{problem}[Problem 46: Power Mean, Young's, and AM-GM]
The numbers $p, q$ and $s$ are fixed and positive. Also $p > 1, q > 1$ and $p = \frac{q}{q-1}$.
...
So $f(t) \geq 0$ for all $t > 0$.
	...
	extbf{(iii)} Induction for AM-GM: Base case $n=1$ is trivial. Assume for $k$, prove for $k+1$ by grouping and using the hypothesis. (See detailed proof in sample.)
...
\item Induction is a powerful tool for inequalities
\item Cyclic sums often reduce to AM-GM
\end{takeaways}

% ------------------------------------------------------------------------------

\begin{problem}[Problem 47: Inductive Proof of AM-GM]
