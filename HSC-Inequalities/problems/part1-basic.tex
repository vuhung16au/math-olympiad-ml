...
\begin{solution}
\textbf{Part 1: Proving the Integral Inequality}

Consider the function $f(x) = \frac{1}{x}$. For $x > 0$, this is a strictly decreasing function. Therefore, for the interval $n \le x \le n+1$ (where $n > 0$), we have:
\[
\frac{1}{n+1} \le \frac{1}{x} \le \frac{1}{n}
\]
Since the function is strictly decreasing, the area under the curve $\frac{1}{x}$ from $x=n$ to $x=n+1$ is strictly bounded by the area of the lower rectangle (height $\frac{1}{n+1}$) and the upper rectangle (height $\frac{1}{n}$).

Integrating with respect to $x$ from $n$ to $n+1$:
\[
\int_{n}^{n+1} \frac{1}{n+1} \, dx < \int_{n}^{n+1} \frac{1}{x} \, dx < \int_{n}^{n+1} \frac{1}{n} \, dx
\]
Since $\frac{1}{n+1}$ and $\frac{1}{n}$ are constants with respect to $x$, and the interval length is $(n+1) - n = 1$:
\begin{align*}
\frac{1}{n+1} \cdot 1 &< \int_{n}^{n+1} \frac{1}{x} \, dx < \frac{1}{n} \cdot 1 \\
\frac{1}{n+1} &< \left[ \ln x \right]_{n}^{n+1} < \frac{1}{n} \\
\frac{1}{n+1} &< \ln(n+1) - \ln(n) < \frac{1}{n}
\end{align*}

\textbf{Part 2: Deducing the Bounds of $e$}

Using logarithm laws, we rewrite the middle term:
\[
...
1 &< (n+1)\ln\left(1+\frac{1}{n}\right) \quad \text{(Multiply by } n+1 \text{)} \\
1 &< \ln\left[\left(1+\frac{1}{n}\right)^{n+1}\right] \\
e^1 &< \left(1+\frac{1}{n}\right)^{n+1} \quad \text{(Exponentiate both sides)}
\end{align*}

\textbf{Right Inequality:}
\begin{align*}
\ln\left(1+\frac{1}{n}\right) &< \frac{1}{n} \\
n\ln\left(1+\frac{1}{n}\right) &< 1 \quad \text{(Multiply by } n \text{)} \\
\ln\left[\left(1+\frac{1}{n}\right)^{n}\right] &< 1 \\
\left(1+\frac{1}{n}\right)^{n} &< e^1 \quad \text{(Exponentiate both sides)}
\end{align*}
