...
The Cauchy-Schwarz Inequality is a powerful mathematical principle that is widely known for its application to numbers and discrete vectors. However, it is equally profound and useful when applied to continuous functions through definite integrals.

\textbf{Part (i)}
Let $f(x)$ and $g(x)$ be continuous real-valued functions on the interval $[a, b]$.
By considering the fact that $\int_a^b (t f(x) + g(x))^2 \, dx \ge 0$ for all real values of $t$, and analyzing the discriminant of the resulting quadratic equation in terms of $t$, prove the Cauchy-Schwarz inequality for definite integrals:
...
Prove that $\int_0^2 (P'(x))^2 \, dx \ge 8$.
\end{problem}

\begin{solution}
\textit{Hints for solving:}
\begin{itemize}
...
\textbf{Part (i) - Solution 1: The Discriminant Method (Standard)}
...
\textbf{Part (i) - Solution 2: The Double Integral Method (Alternative)}
