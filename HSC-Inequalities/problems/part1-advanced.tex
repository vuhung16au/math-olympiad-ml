...
Let $t$ be any real number. We have: 
$$\int_a^b (t f(x) + g(x))^2 \, dx \ge 0$$
...
Because this quadratic is greater than or equal to zero for all real values of $t$, its discriminant must be less than or equal to zero ($\Delta \le 0$).
...
By putting the second term to the right and side 
and dividing by $4$ yields the Cauchy-Schwarz inequality. $\blacksquare$
...
\begin{takeaways}
    ...
    \item The equality holds in part (i) when $f(x) = k g(x)$ for some constant $k$.
\end{takeaways}
