...
% Note: We skip dl101-boxes because it uses chapter numbering which doesn't work with article class
% Instead, we'll define our own boxes with section numbering
\usepackage[most]{tcolorbox}
\usepackage{geometry}

% Force English figure names (vntex sets Vietnamese by default)
...
% Define boxes with section numbering (for article class, not chapter-based)
\newtcolorbox[auto counter,number within=section]{theorembox}[1][]{
  title={\bfseries Theorem~\thetcbcounter\ifx\relax#1\relax\else: #1\fi},
  enhanced,
...
\newtcolorbox[auto counter,number within=section]{examplebox}[1][]{
  title={\bfseries Example~\thetcbcounter\ifx\relax#1\relax\else: #1\fi},
  enhanced,
...
\newtcolorbox[auto counter,number within=section]{remarkbox}[1][]{
  title={\bfseries Remark~\thetcbcounter\ifx\relax#1\relax\else: #1\fi},
  enhanced,
  drop shadow={black!20!white},
  coltitle=bookwhite,
  colback=softivory,
...
\newtcolorbox[auto counter,number within=section]{definitionbox}[1][]{
  title={\bfseries Definition~\thetcbcounter\ifx\relax#1\relax\else: #1\fi},
  enhanced,
  drop shadow={black!20!white},
  coltitle=bookwhite,
  colback=softivory,
...
% Redefine theorem environments to use the boxes
\renewenvironment{theorem}[1][]{\begin{theorembox}[#1]}{\end{theorembox}}
\renewenvironment{example}[1][]{\begin{examplebox}[#1]}{\end{examplebox}}
\renewenvironment{remark}[1][]{\begin{remarkbox}[#1]}{\end{remarkbox}}
\renewenvironment{definition}[1][]{\begin{definitionbox}[#1]}{\end{definitionbox}}

% Solution environment - styled as a box
\newtcolorbox[auto counter,number within=section]{solutionbox}[1][]{
  title={\bfseries Solution\ifx\relax#1\relax\else: #1\fi},
  enhanced,
  drop shadow={black!20!white},
  coltitle=bookwhite,
  colback=softivory,
...
% Define solution environment to use the box
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[
        ...
        \node[point, label=above:$E$] (E) at (-1.5, -1.325) {};
        % Right side (x > 0): D, B, F
        \node[point, label=right:$D$] (D) at (1.5, -1.325) {};
        \node[point, label=above:$B$] (B) at (1, -1.7) {};
        \node[point, label=above:$F$] (F) at (2, -0.8) {};
    ...
    \begin{tikzpicture}[
        ...
        % Points on hyperbola - calculated to lie exactly on the curve
        ...
        \node[point, label=left:$C$] (C) at (-2.0, {0.7*sqrt(2.0*2.0 - 2.25)}) {};
        \node[point, label=below:$E$] (E) at (-2.5, {-0.7*sqrt(2.5*2.5 - 2.25)}) {};
        % Right branch: F, B, D
        \node[point, label=right:$F$] (F) at (2.8, {-0.7*sqrt(2.8*2.8 - 2.25)}) {};
...
\begin{enumerate}
    ...
    \item \textbf{Coordinate Geometry Approach}: Using homogeneous coordinates, 
    we can express the condition that six points lie on a conic as a determinant condition. The collinearity of $X, Y, Z$ can then be verified algebraically.
...
The most elegant modern proof uses the theory of cubic curves. 
The six points on the conic, together with the three intersection points $X, Y, Z$, can be related through cubic curves.
 The fact that the six points lie on a conic (a quadratic curve) forces a relationship that implies $X, Y, Z$ are collinear.
