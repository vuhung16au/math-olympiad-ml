...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[
        ...
        % Points on circle in order A-B-C-D-E-F (hexagon order)
        % Arranged so lines extend outside the circle
        \node[point, label=above:$A$] (A) at (60:2.5) {};
        \node[point, label=above left:$B$] (B) at (120:2.5) {};
        \node[point, label=left:$C$] (C) at (180:2.5) {};
        \node[point, label=below left:$D$] (D) at (240:2.5) {};
        \node[point, label=below:$E$] (E) at (300:2.5) {};
        \node[point, label=right:$F$] (F) at (0:2.5) {};
        
        % Hexagon sides according to Pascal's theorem:
        % X = (AB) ∩ (DE), Y = (BC) ∩ (EF), Z = (CD) ∩ (FA)
        % Extended well beyond circle so intersections occur outside
        \draw[green!70!black, thick, name path=AB] ($(A)!-1.0!(B)$) -- ($(A)!2.0!(B)$);
        \draw[green!70!black, thick, name path=DE] ($(D)!-1.0!(E)$) -- ($(D)!2.0!(E)$);
        
        \draw[blue, thick, name path=BC] ($(B)!-1.0!(C)$) -- ($(B)!2.0!(C)$);
        \draw[blue, thick, name path=EF] ($(E)!-1.0!(F)$) -- ($(E)!2.0!(F)$);
        
        \draw[magenta, thick, name path=CD] ($(C)!-1.0!(D)$) -- ($(C)!2.0!(D)$);
        \draw[magenta, thick, name path=FA] ($(F)!-1.0!(A)$) -- ($(F)!2.0!(A)$);
        ...
        \node[point, label=above left:$X$] at (X) {};
        \node[point, label=below right:$Y$] at (Y) {};
        \node[point, label=above right:$Z$] at (Z) {};
        
        % Draw Pascal line
        \draw[red, dashed, thick] ($(X)!-0.5!(Z)$) -- ($(X)!1.5!(Z)$);
        
        \node[below] at (0, -4.5) {(a1) Circle with lines cutting outside};
    \end{tikzpicture}
    \hfill
    \begin{tikzpicture}[
        point/.style={circle, fill, inner sep=1.5pt},
        ...
        % Points on circle in order A-B-C-D-E-F (hexagon order)
        % Arranged so lines intersect inside the circle
        \node[point, label=above:$A$] (A) at (30:2.5) {};
        \node[point, label=above left:$B$] (B) at (90:2.5) {};
        \node[point, label=left:$C$] (C) at (150:2.5) {};
        \node[point, label=below left:$D$] (D) at (210:2.5) {};
        \node[point, label=below:$E$] (E) at (270:2.5) {};
        \node[point, label=right:$F$] (F) at (330:2.5) {};
        
        % Hexagon sides according to Pascal's theorem:
        % X = (AB) ∩ (DE), Y = (BC) ∩ (EF), Z = (CD) ∩ (FA)
        % Extended slightly so intersections are clearly visible inside the circle
        \draw[green!70!black, thick, name path=AB] ($(A)!-0.2!(B)$) -- ($(A)!1.2!(B)$);
        \draw[green!70!black, thick, name path=DE] ($(D)!-0.2!(E)$) -- ($(D)!1.2!(E)$);
        
        \draw[blue, thick, name path=BC] ($(B)!-0.2!(C)$) -- ($(B)!1.2!(C)$);
        \draw[blue, thick, name path=EF] ($(E)!-0.2!(F)$) -- ($(E)!1.2!(F)$);
        
        \draw[magenta, thick, name path=CD] ($(C)!-0.2!(D)$) -- ($(C)!1.2!(D)$);
        \draw[magenta, thick, name path=FA] ($(F)!-0.2!(A)$) -- ($(F)!1.2!(A)$);
        ...
        \node[below] at (0, -4.5) {(a2) Circle with lines cutting inside};
    \end{tikzpicture}
...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[
        ...
        % Draw double cone (simplified 2D representation)
        % Upper cone
        \filldraw[fill=yellow!30, draw=black, thick, opacity=0.7] 
            (0, \apexY) -- (-\coneRadius, \apexY + \coneHeight) -- 
            (\coneRadius, \apexY + \coneHeight) -- cycle;
        % Lower cone
        \filldraw[fill=yellow!30, draw=black, thick, opacity=0.7] 
            (0, \apexY) -- (-\coneRadius, \apexY - \coneHeight) -- 
            (\coneRadius, \apexY - \coneHeight) -- cycle;
        ...
        \filldraw[blue!70!black] (0, \apexY) circle (2.5pt);
        
        % Circle: horizontal plane (perpendicular to axis)
        \draw[thick, blue, line width=1.5pt] (-1.3, \apexY - 2.8) -- (1.3, \apexY - 2.8);
        ...
        \draw[thick, green!70!black, line width=1.5pt] (-1.6, \apexY - 1.5) -- (1.6, \apexY + 1.0);
        \draw[thick, green!70!black, line width=1.5pt] (0, \apexY - 0.25) ellipse (1.6cm and 0.7cm);
        \node[green!70!black, right, font=\small] at (1.8, \apexY + 0.4) {\textbf{ellipse}};
        
        % Parabola: plane parallel to one side of cone
        \draw[thick, red!70!black, line width=1.5pt] (-2.0, \apexY + 1.8) -- (0.6, \apexY + 3.8);
        % Draw parabola curve (opening upward)
        \draw[thick, red!70!black, line width=1.5pt, domain=-1.3:0.4, samples=100] 
            plot (\x, {2.8 + 0.9*(\x + 0.45)^2});
        \node[red!70!black, right, font=\small] at (0.8, \apexY + 3.0) {\textbf{parabola}};
        
        % Hyperbola: vertical plane cutting both cones
        \draw[thick, magenta!70!black, line width=1.5pt] (0.9, \apexY - \coneHeight) -- (0.9, \apexY + \coneHeight);
        % Upper branch (right side)
        \draw[thick, magenta!70!black, line width=1.5pt, domain=0.9:2.4, samples=100] 
            plot (\x, {\apexY + 1.6 + 0.7*sqrt((\x - 0.9)^2)});
...
Several directions for further exploration emerge from Pappus's hexagon theorem:

\begin{enumerate}
    \item \textbf{Generalizations}: Pappus's hexagon theorem can be generalized to configurations with more points and lines. The study of such configurations leads to matroid theory and combinatorial geometry.
    ...
    \item \textbf{Finite Projective Planes}: Pappus's hexagon theorem holds in finite projective planes as well. The study of finite geometries and their automorphism groups is an active area of research.
    ...
    \item \textbf{Applications in Computer Vision}: Projective geometry, including results like Pappus's hexagon theorem, has applications in camera calibration and 3D reconstruction from images.
    
    \item \textbf{Connection to Other Theorems}: Exploring relationships between Pappus's hexagon theorem, Desargues's Theorem, and other fundamental results in projective geometry reveals deep structural connections.
    ...
    \item \textbf{Cayley–Bacharach Theorem}: This fundamental result in algebraic geometry states that if two cubic curves intersect in nine distinct points, then any cubic curve passing through eight of these points must also pass through the ninth. This theorem generalizes Pascal's Theorem and provides a deeper understanding of the intersection properties of curves in the projective plane. Exploring the connections between Pappus's hexagon theorem and the Cayley–Bacharach Theorem offers insights into the unified structure of projective geometry.
    
    \item \textbf{Bézout's Theorem}: This cornerstone theorem in algebraic geometry asserts that two projective plane curves of degrees $d_1$ and $d_2$ without a common component intersect in exactly $d_1 \times d_2$ points, counted with multiplicity. Bézout's Theorem provides the theoretical foundation for understanding intersection multiplicities and is essential for proving many results in algebraic geometry, including those related to conic sections and their degenerations.
\end{enumerate}
