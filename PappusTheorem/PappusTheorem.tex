...
\begin{figure}[h!]
    ...
    \begin{tikzpicture}[
        ...
        % Points on circle in order around circle: A -> B -> C -> D -> E -> F -> A
        % Hexagon order for Pascal's theorem: A-B-C-D-E-F
        % Arranged as non-regular hexagon so opposite sides clearly intersect outside
        % Angles chosen to ensure AB∩DE, BC∩EF, CD∩FA are all visible
        \node[point, label=above:$A$] (A) at (10:2.5) {};   % 
        \node[point, label=above right:$B$] (B) at (80:2.5) {};   % 
        \node[point, label=above left:$C$] (C) at (140:2.5) {};  % 
        \node[point, label=below left:$D$] (D) at (190:2.5) {};  % 
        \node[point, label=below:$E$] (E) at (260:2.5) {};  % 
        \node[point, label=below right:$F$] (F) at (310:2.5) {};  % 
        
        % Hexagon sides according to Pascal's theorem:
        % X = (AB) ∩ (DE), Y = (BC) ∩ (EF), Z = (CD) ∩ (FA)
        % Extended well beyond circle so intersections occur outside and are visible
        \draw[green!70!black, thick, name path=AB] ($(A)!-2.0!(B)$) -- ($(A)!3.0!(B)$);
        \draw[green!70!black, thick, name path=DE] ($(D)!-2.0!(E)$) -- ($(D)!3.0!(E)$);
        
        \draw[blue, thick, name path=BC] ($(B)!-2.0!(C)$) -- ($(B)!3.0!(C)$);
        \draw[blue, thick, name path=EF] ($(E)!-2.0!(F)$) -- ($(E)!3.0!(F)$);
        
        \draw[magenta, thick, name path=CD] ($(C)!-2.0!(D)$) -- ($(C)!3.0!(D)$);
        \draw[magenta, thick, name path=FA] ($(F)!-2.0!(A)$) -- ($(F)!3.0!(A)$);
