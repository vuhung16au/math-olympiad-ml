...
% =============================================================================
% PROBLEM 5: Power Reduction for Trigonometric Integration
% Technique: Double angle formulas, power reduction, definite integration
% =============================================================================

\begin{problem}[Power Reduction Method]
\begin{enumerate}[label=(\roman*)]
\item Using the double angle formula $\cos(2\theta) = 2\cos^2\theta - 1$, show that
\[
\cos^2\theta = \frac{1}{2}(1 + \cos(2\theta))
\]
\item Hence evaluate $\displaystyle \int_0^{\frac{\pi}{2}} \cos^2\theta \, d\theta$.
\item By repeatedly applying the identity in part (i), or otherwise, show that
\[
\cos^4\theta = \frac{1}{8}(\cos(4\theta) + 4\cos(2\theta) + 3)
\]
\item Hence evaluate $\displaystyle \int_0^{\frac{\pi}{2}} \cos^4\theta \, d\theta$.
\end{enumerate}
\end{problem}

\begin{hint}
For part (i), rearrange the double angle formula to isolate $\cos^2\theta$. For part (iii), square the result from part (i) and apply the formula again to the $\cos^2(2\theta)$ term. For integration, substitute the reduced form and integrate term by term.
\end{hint}

\begin{solution}
\textbf{Part (i):} From $\cos(2\theta) = 2\cos^2\theta - 1$, rearranging gives:
\[
\cos^2\theta = \boxed{\frac{1}{2}(1 + \cos(2\theta))} \quad \blacksquare
\]

\textbf{Part (ii):}
\begin{align*}
\int_0^{\frac{\pi}{2}} \cos^2\theta \, d\theta &= \int_0^{\frac{\pi}{2}} \frac{1}{2}(1 + \cos(2\theta)) \, d\theta = \frac{1}{2} \left[ \theta + \frac{\sin(2\theta)}{2} \right]_0^{\frac{\pi}{2}} \\
&= \frac{1}{2}\left(\frac{\pi}{2} + 0\right) = \boxed{\frac{\pi}{4}}
\end{align*}

\textbf{Part (iii):} Square the identity from part (i):
\begin{align*}
\cos^4\theta &= \left[\frac{1}{2}(1 + \cos(2\theta))\right]^2 = \frac{1}{4}(1 + 2\cos(2\theta) + \cos^2(2\theta))
\end{align*}
Apply part (i) to $\cos^2(2\theta)$: $\cos^2(2\theta) = \frac{1}{2}(1 + \cos(4\theta))$
\begin{align*}
\cos^4\theta &= \frac{1}{4}\left(1 + 2\cos(2\theta) + \frac{1}{2}(1 + \cos(4\theta))\right) = \frac{1}{4}\left(\frac{3}{2} + 2\cos(2\theta) + \frac{1}{2}\cos(4\theta)\right) \\
&= \boxed{\frac{1}{8}(\cos(4\theta) + 4\cos(2\theta) + 3)} \quad \blacksquare
\end{align*}

\textbf{Part (iv):}
\begin{align*}
\int_0^{\frac{\pi}{2}} \cos^4\theta \, d\theta &= \frac{1}{8} \int_0^{\frac{\pi}{2}} (\cos(4\theta) + 4\cos(2\theta) + 3) \, d\theta \\
&= \frac{1}{8} \left[ \frac{\sin(4\theta)}{4} + 2\sin(2\theta) + 3\theta \right]_0^{\frac{\pi}{2}} = \frac{1}{8} \cdot \frac{3\pi}{2} = \boxed{\frac{3\pi}{16}}
\end{align*}
\end{solution}

\begin{takeaways}
\begin{itemize}
\item \textbf{Power Reduction Strategy:} Use double angle formulas to convert $\cos^n\theta$ to sums of single-angle terms
\item \textbf{Iterative Application:} Apply the same identity multiple times for higher powers ($\cos^2 \to \cos^4$)
\item \textbf{Integration Simplification:} Reduced forms $\int \cos(n\theta)d\theta = \frac{1}{n}\sin(n\theta)$ are much easier than direct powers
\item \textbf{Special Values:} $\sin(0) = \sin(\pi) = \sin(2\pi) = 0$ simplify boundary evaluations
\item \textbf{General Pattern:} This method extends to any even power: $\cos^{2n}\theta$ reduces to multiple-angle cosines
\end{itemize}
\end{takeaways}
