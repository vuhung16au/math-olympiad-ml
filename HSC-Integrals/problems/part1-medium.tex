...
\textbf{Strategy:} This is a classic reduction formula problem. For part (i), we'll combine $I_n + I_{n-1}$ and use the identity $\cot^2\theta + 1 = \csc^2\theta$ to simplify, then apply substitution. For part (ii), we'll use the recurrence relation iteratively, starting from $I_0$.
...
Consider $I_n + I_{n-1}$:
\begin{align*}
I_n + I_{n-1} &= \int_{\frac{\pi}{4}}^{\frac{\pi}{2}} \cot^{2n}\theta \, d\theta + \int_{\frac{\pi}{4}}^{\frac{\pi}{2}} \cot^{2(n-1)}\theta \, d\theta \\
&= \int_{\frac{\pi}{4}}^{\frac{\pi}{2}} \left( \cot^{2n}\theta + \cot^{2n-2}\theta \right) d\theta \\
&= \int_{\frac{\pi}{4}}^{\frac{\pi}{2}} \cot^{2n-2}\theta \left( \cot^2\theta + 1 \right) d\theta
\end{align*}

Using the Pythagorean identity $\cot^2\theta + 1 = \csc^2\theta$:
\[
I_n + I_{n-1} = \int_{\frac{\pi}{4}}^{\frac{\pi}{2}} \cot^{2n-2}\theta \cdot \csc^2\theta \, d\theta
\]

\textit{Substitution:} Let $u = \cot\theta$, then $du = -\csc^2\theta \, d\theta$.

Change limits:
\begin{itemize}
\item When $\theta = \frac{\pi}{4}$: $u = \cot\left(\frac{\pi}{4}\right) = 1$
\item When $\theta = \frac{\pi}{2}$: $u = \cot\left(\frac{\pi}{2}\right) = 0$
\end{itemize}

Substituting:
\begin{align*}
I_n + I_{n-1} &= \int_{1}^{0} u^{2n-2} (-du) \\
&= \int_{0}^{1} u^{2n-2} \, du \\
&= \left[ \frac{u^{2n-1}}{2n-1} \right]_{0}^{1} \\
&= \frac{1}{2n-1}
\end{align*}

Rearranging: $\boxed{I_n = \frac{1}{2n-1} - I_{n-1}}$

\textbf{Part (ii):}

We need $I_2$. First, calculate $I_0$ and $I_1$.

\textit{Calculate $I_0$:}
\[
I_0 = \int_{\frac{\pi}{4}}^{\frac{\pi}{2}} 1 \, d\theta = \left[ \theta \right]_{\frac{\pi}{4}}^{\frac{\pi}{2}} = \frac{\pi}{2} - \frac{\pi}{4} = \frac{\pi}{4}
\]

\textit{Calculate $I_1$:} Using the formula with $n=1$:
\[
I_1 = \frac{1}{2(1)-1} - I_0 = 1 - \frac{\pi}{4}
\]

\textit{Calculate $I_2$:} Using the formula with $n=2$:
\begin{align*}
I_2 &= \frac{1}{2(2)-1} - I_1 \\
&= \frac{1}{3} - \left( 1 - \frac{\pi}{4} \right) \\
&= \frac{1}{3} - 1 + \frac{\pi}{4} \\
&= \boxed{\frac{\pi}{4} - \frac{2}{3}}
\end{align*}
\end{solution}
...
\textbf{Strategy:} Use King's property (reflection about midpoint) to create a self-referencing equation that simplifies the numerator. Then apply the t-formula (Weierstrass substitution) to handle the trigonometric denominator.
...
\textbf{Strategy:} Use integration by parts with $u = (\ln x)^n$ and $dv = dx$. The derivative of $u$ will reduce the power of the logarithm.
...
\textbf{Strategy:} Apply Newton's Second Law, use the kinematic identity $a = v\frac{dv}{dx}$ to change variables, then separate and integrate.
...
\textbf{Strategy:} Use Euler's formula to convert complex exponentials to trigonometric form, then use binomial theorem and group terms to derive the power reduction formula.
