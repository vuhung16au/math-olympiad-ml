...
\textbf{Strategy:} This sophisticated problem connects integration, reduction formulae, induction, and limits. Part (i) establishes base case, (ii) bounds, (iii) recurrence, (iv) explicit formula via induction, and (v) uses squeeze theorem.
...
\textbf{Strategy:} Transform the second integral using substitution $u = m - x$ to reverse limits and match the form of $A$. Factor out constants carefully.
...
An object of mass $5$ kg is on a slope inclined at $60°$ to the horizontal. The acceleration due to gravity is $g$ m/s$^2$ and velocity down the slope is $v$ m/s.
...
\textbf{Strategy:} Resolve forces parallel to slope using $F = mg\sin\theta$, then apply Newton's First Law for constant velocity (equilibrium).
...
\begin{itemize}
\item \textbf{Down slope:} Component of weight: $F_g = mg\sin(60°) = 5g \cdot \frac{\sqrt{3}}{2} = \frac{5\sqrt{3}}{2}g$
...
\textbf{Strategy:} Use washer method: $V = \pi \int_a^b [(y_{outer})^2 - (y_{inner})^2] dx$. For region $A$, outer is line $y=1$, inner is circle. For region $B$, outer is $y=1$, inner is $\ln x$.
