...
Find a unit vector perpendicular to both $\mathbf{u} = \mathbf{i} + \mathbf{j}$ and $\mathbf{v} = \mathbf{i} + \mathbf{k}$.
\end{problem}
...
Prove that $\cos^2\alpha + \cos^2\beta + \cos^2\gamma = 1$ for a position vector $\mathbf{r} = a\mathbf{i} + b\mathbf{j} + c\mathbf{k}$ making angles $\alpha, \beta, \gamma$ with the axes.
\end{problem}
...
What is the Cartesian equation of $\mathbf{r} = \begin{pmatrix} 1 \\ 3 \end{pmatrix} + \lambda \begin{pmatrix} -2 \\ 4 \end{pmatrix}$? Options: A. $2y + x = 7$, B. $y - 2x = -5$, C. $y + 2x = 5$, D. $2y - x = -1$.
\end{problem}
...
Find the point on line $\mathbf{r} = \begin{pmatrix} 1 \\ 4 \\ 6 \end{pmatrix} + \lambda \begin{pmatrix} 2 \\ 1 \\ 1 \end{pmatrix}$ closest to the origin.
\end{problem}
