...
\begin{enumerate}[label=(\roman*)]
    \item Show that $\overrightarrow{HA} + \overrightarrow{HB} + \overrightarrow{HC} + \overrightarrow{HD} + \overrightarrow{HS} = \vec{0}$.
    
    \item The centroid $G$ of the pyramid is the point such that:
...
\begin{hint}
(i) Use the fact that $H$ is the midpoint of both diagonals. Since the square has symmetry, opposite vertices are equidistant from $H$ in opposite directions.
(ii) Express each $\overrightarrow{GA_i}$ in terms of $\overrightarrow{GH}$ and $\overrightarrow{HA_i}$, then use the result from part (i).
\end{hint}

\begin{solution}
\textbf{Part (i): Sum of vectors from center to all vertices is zero}

Since $H$ is the center of the square base $ABCD$, it is the intersection point of the diagonals $AC$ and $BD$.

For the square base:
\begin{itemize}
    \item $H$ is the midpoint of diagonal $AC$, so $\overrightarrow{HA} = -\overrightarrow{HC}$
    \item $H$ is the midpoint of diagonal $BD$, so $\overrightarrow{HB} = -\overrightarrow{HD}$
\end{itemize}

Therefore, the sum of vectors from $H$ to the four base vertices is:
$$\overrightarrow{HA} + \overrightarrow{HB} + \overrightarrow{HC} + \overrightarrow{HD} = \overrightarrow{HA} + \overrightarrow{HB} + (-\overrightarrow{HA}) + (-\overrightarrow{HB}) = \vec{0}$$

Adding the apex vector:
$$\overrightarrow{HA} + \overrightarrow{HB} + \overrightarrow{HC} + \overrightarrow{HD} + \overrightarrow{HS} = \vec{0} + \overrightarrow{HS} = \overrightarrow{HS}$$

Wait, this shows the sum is $\overrightarrow{HS}$, not zero. Let me reconsider the problem statement.

Actually, the problem asks to show the sum from center to all vertices is zero. Looking at the solution sketch, it seems the problem means the sum of vectors from $H$ to the four base vertices only (not including $S$):
$$\overrightarrow{HA} + \overrightarrow{HB} + \overrightarrow{HC} + \overrightarrow{HD} = \vec{0}$$

This is proven above using the symmetry of the square.

\textbf{Part (ii): Finding the centroid $G$}
...
From part (i), we know $\overrightarrow{HA} + \overrightarrow{HB} + \overrightarrow{HC} + \overrightarrow{HD} = \vec{0}$, so:
...
\textbf{Answer:} The centroid $G$ divides the segment from the center of the base to the apex in the ratio $HG:GS = 1:4$, so $\lambda = \frac{1}{5}$.
\end{solution}
