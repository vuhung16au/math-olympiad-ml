...
\begin{problem}[Concurrent Medians of a Tetrahedron]
In a tetrahedron $ABCD$, define a \emph{median} as the segment from a vertex to the centroid of the opposite face.

Let the position vectors of $A,B,C,D$ be $\mathbf{a},\mathbf{b},\mathbf{c},\mathbf{d}$ relative to origin $O$.
Let $M_A, M_B, M_C, M_D$ be centroids of faces $BCD, ACD, ABD, ABC$ respectively.

\begin{enumerate}[label=(\roman*)]
    \item Show that
    \[
    \mathbf{m}_A=\frac{\mathbf{b}+\mathbf{c}+\mathbf{d}}{3},
    \quad
    \mathbf{m}_B=\frac{\mathbf{a}+\mathbf{c}+\mathbf{d}}{3},
    \quad
    \mathbf{m}_C=\frac{\mathbf{a}+\mathbf{b}+\mathbf{d}}{3},
    \quad
    \mathbf{m}_D=\frac{\mathbf{a}+\mathbf{b}+\mathbf{c}}{3}.
    \]
    \item Define
    \[
    ...
    \item Deduce the internal division ratio on each median.
\end{enumerate}
\end{problem}

\begin{hint}
Rewrite $\mathbf{g}$ by isolating one vertex and grouping the other three. For example:
$\mathbf{g}=\frac{\mathbf{a}+(\mathbf{b}+\mathbf{c}+\mathbf{d})}{4}
=\frac{1}{4}\mathbf{a}+\frac{3}{4}\mathbf{m}_A$.
Then repeat by symmetry for $B,C,D$.
\end{hint}
...
By symmetry:
\[
\mathbf{g}=\frac{1}{4}\mathbf{b}+\frac{3}{4}\mathbf{m}_B
=\frac{1}{4}\mathbf{c}+\frac{3}{4}\mathbf{m}_C
=\frac{1}{4}\mathbf{d}+\frac{3}{4}\mathbf{m}_D,
\]
...
\begin{tikzpicture}[scale=1.0, line join=round, line cap=round]
    ...
    \coordinate (O) at (0,0);
    \coordinate (A) at (5,0);
    \coordinate (Q) at (1.5,4.5);

    % Points from given ratios
    \coordinate (P) at (3,0);          % OP : OA = 3 : 5
    \coordinate (B) at (0.5,1.5);      % OB : OQ = 1 : 3
    \coordinate (R) at (2.75,0.75);    % intersection of AB and PQ
    \coordinate (T) at (4.125,1.125);  % intersection of AQ and OR

    % Triangle and internal lines
    \draw[thick] (O) -- (A) -- (Q) -- cycle;
    \draw[thick, teal!70!black] (A) -- (B);
    \draw[thick, orange!85!black] (P) -- (Q);
    \draw[dashed, purple!80!black] (O) -- (T);

    % Vector arrows
    \draw[->, very thick, blue!70!black] (O) -- (A) node[midway, below] {$\mathbf{a}$};
    \draw[->, very thick, red!75!black] (O) -- (B) node[midway, left] {$\mathbf{b}$};
...
By symmetry of a regular octagon, the 8 vertices are evenly distributed around the center $O$. The vectors $\overrightarrow{OP_1}, \overrightarrow{OP_2}, \ldots, \overrightarrow{OP_8}$ point in directions separated by $45^\circ$ and all have equal magnitude. Their vector sum is:
...
The apothem is the perpendicular distance from center to midpoint of a side. Consider the right triangle formed by $O$, midpoint $A$, and vertex $P_1$. The central angle between adjacent vertices is $45^\circ$, so half this angle at $O$ is $22.5^\circ$. With opposite side $|AP_1| = 2$ (half the side length) and adjacent side $|AO|$:
$$\tan(22.5^\circ) = \frac{2}{|AO|} \quad \Rightarrow \quad |AO| = 2\cot(22.5^\circ)$$

\textbf{Step 4: Evaluate $\cot(22.5^\circ)$.}
