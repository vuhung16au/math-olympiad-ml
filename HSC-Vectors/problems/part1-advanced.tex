...
\begin{tikzpicture}[tdplot_main_coords,scale=1.2]
  ...
  % Mark vertices
  \node[circle,fill=black,inner sep=2pt,label=below left:$A$] at (A) {};
  \node[circle,fill=black,inner sep=2pt,label=below right:$B$] at (B) {};
  \node[circle,fill=black,inner sep=2pt,label=above:$C$] at (C) {};
  \node[circle,fill=black,inner sep=2pt,label=above:$D$] at (D) {};
  
  % Mark midpoints
  \node[circle,fill=blue,inner sep=1.5pt,label=below:$L$] at (L) {};
  \node[circle,fill=red,inner sep=1.5pt,label=left:$M$] at (M) {};
  \node[circle,fill=green!60!black,inner sep=1.5pt,label=left:$N$] at (N) {};
  \node[circle,fill=blue,inner sep=1.5pt,label=above right:$P$] at (P) {};
  \node[circle,fill=red,inner sep=1.5pt,label=right:$Q$] at (Q) {};
  \node[circle,fill=green!60!black,inner sep=1.5pt,label=below right:$R$] at (R) {};
  
  % Label edges with vectors
  \draw[->,thick,purple] (A) -- ($(A)!0.4!(B)$) node[midway,below] {\small $\mathbf{b}$};
  \draw[->,thick,orange] (A) -- ($(A)!0.35!(C)$) node[midway,left] {\small $\mathbf{c}$};
  \draw[->,thick,cyan!70!black] (A) -- ($(A)!0.35!(D)$) node[midway,left] {\small $\mathbf{d}$};
\end{tikzpicture}
\end{center}
