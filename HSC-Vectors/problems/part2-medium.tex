...
\begin{solution}[Sketch]
\textbf{Step 1: Apply the perpendicularity condition}

The direction vectors are $\mathbf{d}_1 = \begin{pmatrix} 1 \\ 2 \\ 1 \end{pmatrix}$ and $\mathbf{d}_2 = \begin{pmatrix} 2 \\ q \\ -1 \end{pmatrix}$.

For perpendicular lines: $\mathbf{d}_1 \cdot \mathbf{d}_2 = 0$
$$(1)(2) + (2)(q) + (1)(-1) = 0$$
$$2 + 2q - 1 = 0$$
$$2q = -1$$
$$q = -\frac{1}{2}$$
...
\textbf{Step 2: Apply the intersection condition}

For the lines to intersect, there must exist values $\mu$ and $\lambda$ such that:
$$\begin{pmatrix} 3 \\ -1 \\ 7 \end{pmatrix} + \mu \begin{pmatrix} 1 \\ 2 \\ 1 \end{pmatrix} = \begin{pmatrix} 1 \\ 0 \\ p \end{pmatrix} + \lambda \begin{pmatrix} 2 \\ q \\ -1 \end{pmatrix}$$

Substituting $q = -\frac{1}{2}$, we get the system:
\begin{align*}
3 + \mu &= 1 + 2\lambda \quad \text{(x-component)} \\
-1 + 2\mu &= 0 - \frac{1}{2}\lambda \quad \text{(y-component)} \\
7 + \mu &= p - \lambda \quad \text{(z-component)}
\end{align*}

\vspace{0.3em}

\textbf{Step 3: Solve for $\mu$ and $\lambda$}

From the first equation: $\mu = 2\lambda - 2$.

Substitute into the second equation:
$$-1 + 2(2\lambda - 2) = -\frac{1}{2}\lambda$$
$$-1 + 4\lambda - 4 = -\frac{1}{2}\lambda$$
$$4\lambda + \frac{1}{2}\lambda = 5$$
$$\frac{9}{2}\lambda = 5$$
$$\lambda = \frac{10}{9}$$

Therefore: $\mu = 2 \cdot \frac{10}{9} - 2 = \frac{20}{9} - \frac{18}{9} = \frac{2}{9}$.

\vspace{0.3em}

\textbf{Step 4: Find $p$ using the z-component equation}

$$7 + \mu = p - \lambda$$
$$p = 7 + \mu + \lambda = 7 + \frac{2}{9} + \frac{10}{9}$$
$$p = 7 + \frac{12}{9} = 7 + \frac{4}{3} = \frac{25}{3}$$
...
\begin{tikzpicture}[scale=0.85]
    ...
    \begin{scope}[xshift=0cm]
        ...
        \node[below, red, font=\small] at (P) {$E$};
        \node[right, red, font=\small] at (Q) {$F$};
        \node[above, red, font=\small] at (R) {$G$};
        \node[left, red, font=\small] at (S) {$H$};
    ...
    \begin{scope}[xshift=5.5cm]
        ...
        \fill[red] (R) circle (1.5pt);
        \fill[red] (S) circle (1.5pt);
        \node[left, red, font=\small] at (P) {$E$};
        \node[above right, red, font=\small] at (Q) {$B$};
        \node[right, red, font=\small] at (R) {$F$};
        \node[left, red, font=\small] at (S) {$H$};
    ...
    \begin{scope}[xshift=11cm]
        ...
        \fill[red] (S) circle (1.5pt);
        \node[left, red, font=\small] at (P) {$E$};
        \node[above right, red, font=\small] at (Q) {$F$};
        \node[below right, red, font=\small] at (R) {$G$};
        \node[left, red, font=\small] at (S) {$H$};
...
\begin{tikzpicture}[scale=1.2, x={(0.8cm, 0.4cm)}, y={(0cm, 1cm)}, z={(-0.6cm, 0.3cm)}]
    ...
    % A at origin, B forward-right, C up-forward-right, D up-back-left
    ...
    \draw[thick, black!50, dashed] (A) -- (D);
    \draw[thick, black!50, dashed] (D) -- (C);
    ...
    \draw[gray!40, dashed, ->] (0, 0, 0) -- (0, 3, 0) node[above, font=\tiny] {$y$};
    \draw[gray!40, dashed, ->] (0, 0, 0) -- (0, 0, 2.5) node[below left, font=\tiny] {$z$};
    ...
    \node[above right, red, font=\small] at (R) {$R$};
    \node[left, red, font=\small] at (S) {$S$};
    
    % Annotation
    \node[font=\small, align=center] at (1.5, -0.8, 1) {Non-planar quadrilateral in $\mathbb{R}^3$\\
    {\footnotesize (vertices not in the same plane)}};
\end{tikzpicture}
\end{center}
...
\begin{itemize}
    ...
    \item \textbf{Verification Strategy:} Always substitute final values back into the original equations to verify correctness. Here: $2(\frac{15}{8}) + \frac{15}{8} - \frac{5}{8} = \frac{30 + 15 - 5}{8} = \frac{40}{8} = 5$ ✓
...
\textbf{Verification:} $\overrightarrow{OA} = (2,2,1)$ and $\overrightarrow{BC_1} = (3,4,3) - (1,2,2) = (2,2,1)$ ✓
...
\textbf{Verification:} $\overrightarrow{OA} = (2,2,1)$ and $\overrightarrow{C_2B} = (1,2,2) - (1,0,-1) = (0,2,3)$... Actually, $\overrightarrow{AC_2} = (1,0,-1) - (2,2,1) = (-1,-2,-2)$ and $\overrightarrow{OB} = (1,2,2)$. Let me verify: $\overrightarrow{OC_2} + \overrightarrow{OB} = (1,0,-1) + (1,2,2) = (2,2,1) = \overrightarrow{OA}$ ✓
...
\textbf{Verification:} $\overrightarrow{OB} = (1,2,2)$ and $\overrightarrow{C_3A} = (2,2,1) - (-1,0,1) = (3,2,0)$... Let me verify: $\overrightarrow{OC_3} + \overrightarrow{OA} = (-1,0,1) + (2,2,1) = (1,2,2) = \overrightarrow{OB}$ ✓
