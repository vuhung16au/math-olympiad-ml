...
(b) A plane passes through the fixed point $P(a, b, c)$ where $a,b,c > 0$. The plane cuts the positive coordinate axes at $X(x_0,0,0), Y(0,y_0,0), Z(0,0,z_0)$.
Show that the minimum sum of the intercepts $S = x_0 + y_0 + z_0$ is $(\sqrt{a} + \sqrt{b} + \sqrt{c})^2$.
...
\begin{hint}
Apply Cauchy-Schwarz to vectors formed by coordinates and reciprocals for part (a). For part (b), apply Cauchy-Schwarz directly to the constraint equation to minimize the sum.
\end{hint}

\begin{solution}
\textbf{(a) Proof:}
By Cauchy--Schwarz on vectors $(\sqrt{x}, \sqrt{y}, \sqrt{z})$ and $(\frac{1}{\sqrt{x}}, \frac{1}{\sqrt{y}}, \frac{1}{\sqrt{z}})$:
\[
    (x+y+z)\Big(\frac{1}{x}+\frac{1}{y}+\frac{1}{z}\Big) \ge \left( \sqrt{x} \cdot \frac{1}{\sqrt{x}} + \sqrt{y} \cdot \frac{1}{\sqrt{y}} + \sqrt{z} \cdot \frac{1}{\sqrt{z}} \right)^2 = (1+1+1)^2 = 9
\]
with equality when $x=y=z$.

\textbf{(b) Application:}
The equation of the plane is $\frac{x}{x_0} + \frac{y}{y_0} + \frac{z}{z_0} = 1$.
Since the plane passes through $P(a,b,c)$:
\[ \frac{a}{x_0} + \frac{b}{y_0} + \frac{c}{z_0} = 1 \]
We want to minimize the sum $S = x_0 + y_0 + z_0$.
Applying Cauchy-Schwarz to the vectors $(\sqrt{x_0}, \sqrt{y_0}, \sqrt{z_0})$ and $(\sqrt{\frac{a}{x_0}}, \sqrt{\frac{b}{y_0}}, \sqrt{\frac{c}{z_0}})$:
\begin{align*}
    (x_0 + y_0 + z_0)\left(\frac{a}{x_0} + \frac{b}{y_0} + \frac{c}{z_0}\right) &\ge (\sqrt{a} + \sqrt{b} + \sqrt{c})^2 \\
    S \cdot (1) &\ge (\sqrt{a} + \sqrt{b} + \sqrt{c})^2
\end{align*}
Thus, the minimum sum of the intercepts is $(\sqrt{a} + \sqrt{b} + \sqrt{c})^2$. \hfill $\square$
\end{solution}
...
By AM-GM on $(x+y+z)$ and $(\frac{1}{x}+\frac{1}{y}+\frac{1}{z})$ separately:
\[
    (x+y+z)\left(\frac{1}{x} + \frac{1}{y} + \frac{1}{z}\right) \ge \left(3\sqrt[3]{xyz}\right) \left(\frac{3}{\sqrt[3]{xyz}}\right) = 9
\]
This gives the same bound using products instead of vector projections.
\end{remark}
...
The prism is inscribed in a sphere of radius $R$, meaning the space diagonal of the prism equals the diameter of the sphere ($2R$). By applying the Pythagorean theorem twice (once for the base diagonal, and once using the height to find the space diagonal), the square of the space diagonal is $x^2 + y^2 + z^2$.
...
\begin{enumerate}[label=(\roman*)]
    ...
    Since $|\alpha|=1$, we know $\bar{\alpha} = \alpha^{-1}$. Expanding the numerator yields:
    $$(\alpha^{n+1} - \alpha^{-n})(\alpha^{-1} - 1) = \alpha^n - \alpha^{n+1} - \alpha^{-n-1} + \alpha^{-n} = \alpha^n + \alpha^{-n} - (\alpha^{n+1} + \alpha^{-(n+1)})$$
    For the denominator, expanding yields:
    $$(\alpha - 1)(\bar{\alpha} - 1) = \alpha\bar{\alpha} - \alpha - \bar{\alpha} + 1 = |\alpha|^2 - (\alpha + \bar{\alpha}) + 1 = 1 - 2\cos\theta + 1 = 2(1-\cos\theta)$$
    Substituting these back into the expression for $C$:
    $$C = \frac{\alpha^n + \alpha^{-n} - (\alpha^{n+1} + \alpha^{-(n+1)})}{(1-\alpha)(1-\bar{\alpha})}$$
...
\]
Subtracting $\sqrt2$ and expanding the numerator to form a perfect square yields:
\[a_{k+1}-\sqrt2 = \frac{a_k^2+2}{2a_k} - \frac{2\sqrt{2}a_k}{2a_k} = \frac{a_k^2 - 2\sqrt{2}a_k + 2}{2a_k} = \frac{(a_k-\sqrt2)^2}{2a_k}.
\]
...
\begin{solution}
	\textbf{(i)} Use integration by parts with $u=x^n$ and $dv=(x+1)^{1/2}\,dx$, which gives $du = n x^{n-1}\,dx$ and $v=\frac{2}{3}(x+1)^{3/2}$.
\begin{align*}
I_n &= \left[ \frac{2}{3}x^n(x+1)^{3/2} \right]_{-1}^0 - \int_{-1}^0 \frac{2n}{3} x^{n-1}(x+1)^{3/2}\,dx \\
&= 0 - \frac{2n}{3} \int_{-1}^0 x^{n-1}(x+1)\sqrt{x+1}\,dx \\
&= -\frac{2n}{3} \int_{-1}^0 (x^n\sqrt{x+1} + x^{n-1}\sqrt{x+1})\,dx \\
&= -\frac{2n}{3} (I_n + I_{n-1})
\end{align*}
Multiplying by 3 and gathering the $I_n$ terms to the left side:
\begin{align*}
3I_n &= -2n I_n - 2n I_{n-1} \\
(2n+3)I_n &= -2n I_{n-1} \\
I_n &= \frac{-2n}{2n+3}I_{n-1}.
\end{align*}
...
% Problem 15: Mandelbrot Escape Criterion (Sample 15)
\input{problems/15}