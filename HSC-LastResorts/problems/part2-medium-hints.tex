...
% Problem: Ramanujan Summation and Roots of Unity Filters (Sample 01)
\begin{problem}[{\samanfont The Ramanujan Summation \& Filters}]
\textbf{(a)} {\samanfont The Indian mathematician Srinivasa Ramanujan} discovered the following series for $1/\pi$:
\[
\frac{1}{\pi} = \frac{2\sqrt{2}}{9801} \sum_{n=0}^{\infty} u_n (1103 + 26390n), \text{ where } u_n = \dfrac{(4n)!}{(n!)^4 (396)^{4n}}. \text{ Do NOT prove this.}
\]


\begin{enumerate}[label=(\roman*)]
    \item Show that for $n \ge 0$, the ratio of consecutive terms in the sequence $u_n$ is given by:
    \[
    \frac{u_{n+1}}{u_n} = \frac{(4n+1)(4n+3)(2n+1)}{K (n+1)^3}, \text{ where } K = \dfrac{396^4}{8}.
    \]
    

    \item Hence, prove that for all $n \ge 1$:
    \[
    \frac{u_{n+1}}{u_n} < \frac{1}{9 \times 10^9}
    \]

    \item Let $S_k$ be the sum of the first $k$ terms of the series. Using the result from part (ii), explain why the first term alone (i.e., $n=0$) provides an approximation for $\pi$ that is accurate to at least 6 decimal places.
\end{enumerate}

\textbf{(b)} {\samanfont Ramanujan discovered distinct patterns in the partition function, $p(n)$, which counts the number of ways an integer $n$ can be written as a sum. For example, he proved that if $n$ is of the form $5k+4$ (like 4, 9, 14...), then $p(n)$ is always divisible by 5. To isolate these specific terms, mathematicians often use 'Roots of Unity filters.' }

Let $\omega = e^{i \frac{2\pi}{5}}$ be a complex fifth root of unity.

\begin{enumerate}[label=(\roman*)]
    \item Prove the identity:
    \[
    \sum_{r=0}^{4} \omega^{kr} = \begin{cases} 5 & \text{if } k \text{ is a multiple of } 5 \\ 0 & \text{if } k \text{ is not a multiple of } 5 \end{cases}
    \]

    \item Consider the polynomial $P(z) = \sum_{n=0}^{N} a_n z^n$. Using part (i), show that:
    \[
    ...
    % \textit{(This process ``filters'' the series to keep only terms where the power is congruent to $4 \pmod 5$)}.

    \item If $a_n$ represents the partition numbers $p(n)$, and we assume Ramanujan's congruence $p(5k+4) \equiv 0 \pmod 5$ holds, determine the value of:
    \[
...
\begin{hint}
\textbf{Part (a)(i):} Write out $u_{n+1}$ and $u_n$ fully. Remember that $(4(n+1))! = (4n+4)!$. Expand the factorials carefully (e.g., $(4n+4)! = (4n+4)(4n+3)(4n+2)(4n+1)(4n)!$) to cancel terms. Note that $396^4 = (4 \times 99)^4$.

\textbf{Part (a)(ii):} You generally do not need a calculator. Look at the leading terms of the polynomial in the numerator versus the denominator. The denominator contains the massive constant $396^4$. Try to bound the fraction by observing the magnitude when $n=1$.

\textbf{Part (a)(iii):} Think about the error term in a geometric series. If the ratio between consecutive terms is $r$, the sum of the tail is roughly proportional to the first neglected term. If $r$ is tiny (e.g., $10^{-9}$), how fast does the error vanish?

\textbf{Part (b)(i):} This is a geometric series of complex numbers. Recall that the sum of the $n$-th roots of unity is 0, unless the common ratio is exactly 1.

\textbf{Part (b)(ii):} Substitute the summation definition of $P(z)$ into the expression on the LHS. Swap the order of summation (sigma notation) so you sum over $r$ first. You will need to match the exponents of $\omega$.

\textbf{Part (b)(iii):} Use the definition of modular arithmetic. If $p(5k+4)$ is a multiple of 5, what does that imply about the coefficient $a_{5k+4}$?
\end{hint}

\begin{solution}
\textbf{Part (a)}

\textbf{(i)} We write the ratio:
\[
\frac{u_{n+1}}{u_n} = \frac{(4(n+1))!}{((n+1)!)^4 (396)^{4(n+1)}} \times \frac{(n!)^4 (396)^{4n}}{(4n)!}
\]
Regrouping terms:
\[
= \left[ \frac{(4n+4)!}{(4n)!} \right] \left[ \frac{(n!)^4}{((n+1)!)^4} \right] \left[ \frac{1}{396^4} \right]
\]
Simplifying the factorials:
\[
= \frac{(4n+4)(4n+3)(4n+2)(4n+1)}{(n+1)^4 \cdot 396^4}
\]
Factoring $4$ from $(4n+4)$ and $2$ from $(4n+2)$:
\[
= \frac{4(n+1)(4n+3) \cdot 2(2n+1)(4n+1)}{(n+1)^4 \cdot 396^4} = \frac{8(4n+3)(2n+1)(4n+1)}{(n+1)^3 \cdot 396^4}
\]
Thus, $K = 396^4/8$.

\textbf{(ii)} For $n \ge 1$, the numerator grows as $\approx 64n^3$ and denominator as $\approx K n^3$.
Calculate constant: $396^4/8 \approx 3.07 \times 10^9$.
Check $n=1$:
\[
\text{Ratio} \approx \frac{8(7)(3)(5)}{2^3 \cdot (3 \times 10^9)} = \frac{840}{2.4 \times 10^{10}} \approx 3.5 \times 10^{-8}
\]
Since the ratio decreases as $n$ increases, for all $n \ge 1$, ratio $< 10^{-7} < \frac{1}{9 \times 10^9}$.

\textbf{(iii)} The series converges extremely rapidly. The error after the first term ($n=0$) is approximately the magnitude of the second term ($n=1$).
Since $u_1 \approx u_0 \times 10^{-8}$, the contribution of the $n=1$ term affects the 8th decimal place. Thus, the $n=0$ term alone is accurate to at least 6 decimal places.

\textbf{Part (b)}

\textbf{(i)} Sum $S = \sum_{r=0}^{4} (\omega^k)^r$.
If $5 \mid k$, $\omega^k = 1$, sum is $1+1+1+1+1=5$.
If $5 \nmid k$, sum is geometric: $\frac{1((\omega^k)^5 - 1)}{\omega^k - 1}$. Since $\omega^{5k} = 1$, numerator is 0.

\textbf{(ii)}
\[
\sum_{r=0}^{4} \omega^{-4r} P(\omega^r z) = \sum_{r=0}^{4} \omega^{-4r} \sum_{n=0}^{N} a_n \omega^{rn} z^n = \sum_{n=0}^{N} a_n z^n \left( \sum_{r=0}^{4} \omega^{r(n-4)} \right)
\]
From (i), inner sum is 5 if $n-4$ is multiple of 5 (i.e., $n \equiv 4 \pmod 5$), else 0.
So only terms $n=5k+4$ survive:
\[
= 5 \sum_{k} a_{5k+4} z^{5k+4}
\]

\textbf{(iii)} If $a_n = p(n)$ and $p(5k+4) \equiv 0 \pmod 5$, then $a_{5k+4} = 5m$.
Setting $z=1$:
\[
\text{LHS} = 5 \sum_{k} p(5k+4) = 5 \sum_{k} (5m_k) = 25 \sum m_k
\]
Thus the value is a multiple of 25 (specifically, it is $25 \times \text{sum of quotients}$).
\end{solution}

\begin{takeaways}
\begin{enumerate}
    \item \textbf{The History Behind Ramanujan's Series:} This remarkable formula was discovered by Srinivasa Ramanujan around 1914 and recorded in his notebooks. What makes it extraordinary is that Ramanujan provided no proof—he simply stated the result, trusting his incredible mathematical intuition. The full, rigorous proof of this specific formula was not published until 1987, when Jonathan and Peter Borwein finally proved it using the theory of \textbf{Elliptic Modular Functions}. Because the complete proof requires roughly 50 pages of background in modular forms, what we see here is a structural approach that explains exactly where the mysterious numbers $1103$, $26390$, and $9801$ come from using the machinery of \textbf{Singular Moduli}—special values of modular functions that connect number theory, complex analysis, and geometry in profound ways.
    
    \item \textbf{Why This Matters:} Ramanujan's series for $\pi$ is one of the fastest-converging formulas ever discovered. Each term adds approximately 8 decimal digits of precision! This convergence is so rapid that the first term alone gives $\pi$ accurate to at least 6 decimal places. The factorials grow explosively, but the denominator $(396)^{4n}$ is so massive that it completely dominates, creating this incredible convergence rate. Modern supercomputers use similar formulas (derived from Ramanujan's work) to calculate $\pi$ to trillions of digits.
    
    \item \textbf{Roots of Unity Filters:} The technique in part (b) demonstrates how roots of unity can ``filter'' or ``sieve'' polynomial coefficients based on modular arithmetic. This elegant method connects complex number theory to number theory and combinatorics, and it's exactly the kind of technique used to study Ramanujan's partition function congruences. Summing $P(\omega^k)$ over roots of unity is a standard tool in advanced mathematics for extracting specific patterns from infinite series.
    
    \item \textbf{Exam Strategy:} In Q16, if you encounter a complex sequence ratio, rely on algebraic cancellation. Large constants (like $396^4$) usually simplify or dominate the limit, making the analysis more tractable than it first appears.
\end{enumerate}
\end{takeaways}
...
\end{enumerate}
\medskip
	A sequence $a_n$ \emph{converges} to a number $L$ if, for large $n$, the terms $a_n$ get and stay as close to $L$ as we want; we write $a_n\to L$. (The term \emph{limiting sum} refers to series; here we only need the sequence idea.)
\end{problem}
...
\begin{remark}[What we mean by “converge”]
	\textbf{Sequences:} A sequence $a_n$ \emph{converges} to a number $L$ if, for all large $n$, the terms $a_n$ get and stay as close to $L$ as we want. We write $a_n \to L$.

	\textbf{Series / Limiting Sum:} A series $\sum u_n$ has \emph{limiting sum} $S$ if its partial sums $S_N=\sum_{n=1}^N u_n$ get and stay as close to $S$ as we want when $N$ is large; in this case we say the series \emph{converges} to $S$ and write $\sum u_n=S$.

In this book, phrases like “approaches”, “converges to”, and “has limiting sum” all mean “gets and stays very close eventually”. You don’t need the formal $\varepsilon$-definition here—just read “converges” as “gets and stays very close for large $n$”.
\end{remark}
...
\begin{enumerate}
    ...
    \item Similar techniques can be used to find the cube root of any positive real number $x$: 

    $a_{n+1} = \frac{a_n + b_n}{2}, \quad b_{n+1} = \frac{x}{a_{n+1}}$
    with any $a_1 \ge x$ and $b_1 = x/a_1$.

    The speed of convergence is quadratic, like the square root case.

\end{enumerate}
\end{takeaways}

% Problem 6b: Sequence of Prisms (Cube Root — Newton's Method)
\begin{problem}[Converging Prisms — Newton's Method for $2^{1/3}$]
Consider a sequence of rectangular prisms with a square base of side length $a_n$ and height $b_n$. The first prism has $a_1=2$ and $b_1=1$. For integers $n\ge1$ define
\[
a_{n+1}=\frac{2a_n+b_n}{3},\qquad b_{n+1}=\frac{2}{a_{n+1}^2}.
\]

\begin{enumerate}[label=(\roman*)]
    \item Show that for $n\ge2$ every prism in the sequence has volume $2$.
    \item Prove that $a_n\ge\sqrt[3]{2}$ for all $n\ge2$.
    \item Show by induction that for $n\ge1$,
    \[a_n-\sqrt[3]{2}\le\frac{1}{3^{n-2}}\left(a_2-\sqrt[3]{2}\right).
    \]
    \item Deduce that the prisms approach a cube (where $a_n=b_n=\sqrt[3]{2}$) as $n\to\infty$.
    \item (Optional) Use a calculator to find the first five terms of the sequence $a_n$ and discuss the speed of convergence to $\sqrt[3]{2}$.
\end{enumerate}
...
\begin{tikzpicture}[
    ...
    >=stealth]
    % Axes
    \draw[->] (0,0,0) -- (4,0,0) node[right] {$a_n$};
    \draw[->] (0,0,0) -- (0,4,0) node[above] {$a_n$};
    \draw[->] (0,0,0) -- (0,0,5) node[below left] {$b_n$};
    ...
    % Base square
    \draw[thick] (3,0,0) -- (3,3,0) -- (0,3,0);
    
    % Vertical edges
    \draw[thick] (3,0,0) -- (3,0,4);
    \draw[thick] (0,3,0) -- (0,3,4);
    \draw[thick] (3,3,0) -- (3,3,4);
    
    % Top face
    \draw[thick] (3,0,4) -- (3,3,4) -- (0,3,4);
    \draw[thick,dashed] (0,0,4) -- (3,0,4);
    \draw[thick,dashed] (0,0,4) -- (0,3,4);
    
    % Labels
    \node[below] at (1.5,0,0) {$a_n$};
    \node[left] at (0,1.5,0) {$a_n$};
    \node[left] at (0,0,2) {$b_n$};
    \node at (1.5,1.5,2) {$V=2$};
\end{tikzpicture}
\end{center}
\end{problem}

\begin{hint}
For (i) compute $V_{n+1}=a_{n+1}^2 b_{n+1}$ directly. For (ii) apply AM--GM to the three dimensions $a_n, a_n, b_n$ and use the volume from (i). For (iii) express $a_{k+1}-\sqrt[3]{2}$ in terms of $a_k-\sqrt[3]{2}$ and use the fact that $a_k\ge\sqrt[3]{2}$ to bound the error by a constant multiple of the previous error. For (iv) use the squeeze theorem together with the bound in (iii).
\end{hint}

\begin{solution}
	\textbf{(i)} The volume is $V_n=a_n^2 b_n$. Using the recursion,
\[V_{n+1}=a_{n+1}^2 b_{n+1}=a_{n+1}^2\cdot\frac{2}{a_{n+1}^2}=2,
\]
so $V_n\equiv2$ for all $n\ge2$.

	\textbf{(ii)} By AM--GM applied to $a_n, a_n, b_n$:
\[a_{n+1}=\frac{a_n+a_n+b_n}{3}\ge\sqrt[3]{a_n^2 b_n}=\sqrt[3]{2}.\]
Since $a_2^2 b_2=2$, we have $a_2\ge\sqrt[3]{2}$. By induction, $a_n\ge\sqrt[3]{2}$ for all $n\ge2$.

	\textbf{(iii)} From $b_k=2/a_k^2$ we get $a_{k+1}=\frac{2a_k^3+2}{3a_k^2}$. Let $\epsilon_k=a_k-\sqrt[3]{2}$. Then:
\[a_{k+1}-\sqrt[3]{2}=\frac{2a_k^3+2-3\sqrt[3]{2}a_k^2}{3a_k^2}.
\]
Using $a_k^3-(\sqrt[3]{2})^3=\epsilon_k(a_k^2+a_k\sqrt[3]{2}+(\sqrt[3]{2})^2)$ and $a_k^2-(\sqrt[3]{2})^2=\epsilon_k(a_k+\sqrt[3]{2})$, we simplify the numerator:
\begin{align*}
2a_k^3+2-3\sqrt[3]{2}a_k^2 &= 2(a_k^3-(\sqrt[3]{2})^3)-3\sqrt[3]{2}(a_k^2-(\sqrt[3]{2})^2) \\
&= 2\epsilon_k(a_k^2+a_k\sqrt[3]{2}+(\sqrt[3]{2})^2)-3\sqrt[3]{2}\epsilon_k(a_k+\sqrt[3]{2}) \\
&= \epsilon_k(2a_k^2-a_k\sqrt[3]{2}-(\sqrt[3]{2})^2) = \epsilon_k^2(2a_k+\sqrt[3]{2}).
\end{align*}
Hence $\epsilon_{k+1}=\frac{\epsilon_k^2(2a_k+\sqrt[3]{2})}{3a_k^2}$. For $k\ge2$, we have $\sqrt[3]{2}\le a_k\le a_2=\frac{4}{3}$. Since $\epsilon_k\le\frac{4}{3}-\sqrt[3]{2}<\frac{1}{12}$ and $\frac{2a_k+\sqrt[3]{2}}{3a_k^2}\le\frac{2\cdot\frac{4}{3}+\sqrt[3]{2}}{3(\sqrt[3]{2})^2}<\frac{14}{9}$, we get:
\[\epsilon_{k+1}\le\frac{14\epsilon_k^2}{9}<\frac{14\epsilon_k}{9}\cdot\frac{1}{12}=\frac{14\epsilon_k}{108}<\frac{\epsilon_k}{3}.
\]
By induction, $a_n-\sqrt[3]{2}\le\frac{1}{3^{n-2}}(a_2-\sqrt[3]{2})$ for $n\ge1$.

	\textbf{(iv)} From (ii) and (iii), $0\le a_n-\sqrt[3]{2}\le\frac{1}{3^{n-2}}(a_2-\sqrt[3]{2})\to0$ as $n\to\infty$. Since $a_n^2 b_n=2$, we have $b_n\to\sqrt[3]{2}$ as well. Therefore the prisms approach a cube.
\end{solution}

\begin{takeaways}
\begin{enumerate}
    \item \textbf{Newton's General Method:} This problem demonstrates Newton's method in its general form for finding roots. The recurrence $a_{n+1} = \frac{2a_n + b_n}{3}$ where $b_n = \frac{2}{a_n^2}$ is equivalent to applying Newton's method to the function $f(x) = x^3 - 2$ to find $\sqrt[3]{2}$. The general Newton iteration $x_{n+1} = x_n - \frac{f(x_n)}{f'(x_n)}$ yields $x_{n+1} = \frac{2x_n^3 + 2}{3x_n^2} = \frac{2x_n + \frac{2}{x_n^2}}{3}$, which matches our geometric construction.
    
    \item \textbf{Relevance to Taylor Expansion:} The quadratic convergence of Newton's method can be understood through Taylor expansion. If $r$ is the root and $\epsilon_n = a_n - r$ is the error, then expanding $f(a_n) = f(r + \epsilon_n)$ around $r$ gives $f(a_n) \approx f'(r)\epsilon_n + \frac{1}{2}f''(r)\epsilon_n^2$. Since $f(r) = 0$, Newton's iteration yields $\epsilon_{n+1} \approx \frac{f''(r)}{2f'(r)}\epsilon_n^2$, showing the error is proportional to the square of the previous error.
    
    \item \textbf{Speed of Convergence: Quadratic:} Newton's method exhibits quadratic convergence, meaning $\epsilon_{n+1} \approx C \cdot \epsilon_n^2$ for some constant $C$. This is dramatically faster than linear methods (like bisection) where errors only decrease by a constant factor each step. In practice, the number of correct digits roughly doubles with each iteration, making Newton's method one of the most efficient root-finding algorithms.
    
    \item \textbf{Geometric Interpretation:} Just as the square root problem used rectangles converging to a square, this cube root problem uses rectangular prisms (with square bases) converging to a cube. The weighted average $\frac{2a_n + b_n}{3}$ represents the arithmetic mean of the three dimensions, which by AM--GM must be at least the cube root of their product.
    \item Similar techniques can be used to find the cube root of any positive real number $x$: 

    $a_{n+1} = \frac{2a_n + b_n}{3}, \quad b_{n+1} = \frac{x}{a_{n+1}^2}$
