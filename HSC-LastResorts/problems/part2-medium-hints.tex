...
\begin{tikzpicture}[x=0.5cm,y=0.5cm,z=0.3cm,>=stealth]
    ...
    % Draw Plane
    \fill[gray!20, opacity=0.5] (X) -- (Y) -- (Z) -- cycle;
...
Let $\alpha = \cos\theta + i \sin\theta$ and consider the series $C = \alpha^{-n} + \alpha^{-n+1} + \dots + \alpha^{-1} + \alpha^0 + \alpha^1 + \dots + \alpha^n$.

\begin{enumerate}[label=(\roman*)]
    \item Show that $\alpha^k + \alpha^{-k} = 2 \cos k\theta$.
    \item Prove that $C = \frac{\alpha^n + \alpha^{-n} - (\alpha^{n+1} + \alpha^{-(n+1)})}{(1-\alpha)(1-\bar{\alpha})}$.
    \item Deduce that $1 + 2(\cos\theta + \cos 2\theta + \dots + \cos n\theta) = \frac{\cos n\theta - \cos (n+1)\theta}{1 - \cos\theta}$.
    \item Show that $\sum_{k=1}^n \cos \frac{k\pi}{n} = -1$ (independent of $n$).
\end{enumerate}
\end{problem}
