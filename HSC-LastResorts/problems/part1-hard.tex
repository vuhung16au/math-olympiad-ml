...
To test parallelism, compare components:
\[
\frac{2}{1} = 2,\qquad \frac{-1}{1} = -1,\qquad \frac{1}{-2} = -\frac12.
\]
These are not equal, so the direction vectors are not scalar multiples. Hence the lines are not parallel.
...
Let
\[
\mathbf{w} = \mathbf{r}_2(t) - \mathbf{r}_1(s).
\]
For the shortest distance, $\mathbf{w}$ must be perpendicular to both direction vectors:
\[
...
\begin{align*}
\mathbf{w}\cdot \mathbf{d}_1 = 0
&\implies 2(-1 + t - 2s) - (-1 + t + s) + (-4 - 2t - s) = 0 \\
&\implies t + 6s = -5,
\end{align*}
and
\begin{align*}
\mathbf{w}\cdot \mathbf{d}_2 = 0
&\implies (-1 + t - 2s) + (-1 + t + s) - 2(-4 - 2t - s) = 0 \\
&\implies 6t + s = -6.
\end{align*}
Solving,
\[
...
\begin{align*}
P_1 &= (1,2,3) + \left(-\frac{24}{35}\right)(2,-1,1)
= \left(-\frac{13}{35}, \frac{94}{35}, \frac{81}{35}\right),\\
P_2 &= (0,1,-1) + \left(-\frac{31}{35}\right)(1,1,-2)
= \left(-\frac{31}{35}, \frac{4}{35}, \frac{27}{35}\right).
\end{align*}
Therefore
\[
