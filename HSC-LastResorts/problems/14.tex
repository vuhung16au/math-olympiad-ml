...
Proving the irrationality of $\pi^2$ makes the statement stronger, since if $\pi^2$ were irrational, then $\pi$ would also be irrational (as the square root of a irrational number) (do NOT prove this). Thus, by proving that $\pi^2$ is irrational, we also confirm the irrationality of $\pi$.
...
\begin{hint}
\begin{itemize}
    \item \textbf{Part (i):} Expand $(1-x)^n$ using the binomial theorem. Recall that the $k$-th derivative of $\frac{x^m}{n!}$ evaluated at $x=0$ is $0$ if $m \neq k$, and is an integer if $m=k$. Use the symmetry $f(x)=f(1-x)$ for the $x=1$ case.
...
To see why this massive sum collapses, we explicitly evaluate $G''(x) + \pi^2 G(x)$:
\begin{align*}
G''(x) + \pi^2 G(x) &= b^n \sum_{k=0}^n (-1)^k \pi^{2n-2k} f^{(2k+2)}(x) + \pi^2 b^n \sum_{k=0}^n (-1)^k \pi^{2n-2k} f^{(2k)}(x) \\
&= b^n \sum_{k=0}^n (-1)^k \pi^{2n-2k} f^{(2k+2)}(x) + b^n \sum_{k=0}^n (-1)^k \pi^{2n-2k+2} f^{(2k)}(x)
\end{align*}
By expanding these sums, we see that they perfectly telescope. Every term in the first sum cancels with the subsequent term in the second sum. The only term that survives is the $k=0$ term from the second sum (since $f^{(2n+2)}(x) = 0$, the upper boundary term vanishes). This leaves exactly:
$$ G''(x) + \pi^2 G(x) = b^n (-1)^0 \pi^{2n+2} f^{(0)}(x) = \pi^{2n+2} b^n f(x) $$
Substituting this collapsed form back into $H'(x)$:
...
\begin{remark}[The Original Proof by Ivan Niven (1947)]
\textbf{A SIMPLE PROOF THAT $\pi$ IS IRRATIONAL}
...
\]
the positive integer $n$ being specified later. Since $n!f(x)$ has integral coefficients and terms in $x$ of degree not less than $n$, $f(x)$ and its derivatives $f^{(i)}(x)$ have integral values for $x=0$; also for $x=\pi=a/b$, since $f(x) = f(a/b - x)$. By elementary calculus we have
\[
\frac{d}{dx} \{F'(x) \sin x - F(x) \cos x\} = F''(x) \sin x + F(x) \sin x = f(x) \sin x
\]
and
\begin{equation}
\int_0^\pi f(x) \sin x \, dx = \Big[F'(x) \sin x - F(x) \cos x \Big]_0^\pi = F(\pi) + F(0). \tag{*}
\end{equation}
...
\]
so that the integral in (*) is positive, but arbitrarily small for $n$ sufficiently large. Thus (*) is false, and so is our assumption that $\pi$ is rational.

\medskip

\textit{This elegant proof was published by Ivan Niven in the Bulletin of the American Mathematical Society in 1947. It demonstrates the power of combining calculus with number theory to establish fundamental results about irrational numbers.}
\end{remark}
