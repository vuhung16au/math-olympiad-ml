...
\begin{takeaways}
\begin{enumerate}
    \item \textbf{Factoring Strategy:} Recognizing that factoring out the volume product ($abc$) from the surface area expression ($ab+bc+ca$) perfectly isolates the sum of the reciprocals.

    \item \textbf{The Equality Condition:} Emphasizing that optimization using AM-GM relies entirely on the equality condition—the minimum or maximum is strictly achieved when all terms are equal (i.e., $a=b=c$).
\end{enumerate}
\end{takeaways}
...
Separate the expansion into diagonal terms ($\mathbf{v}_i \cdot \mathbf{v}_i$) and mixed terms ($\mathbf{v}_i \cdot \mathbf{v}_j$ for $i \ne j$).
\end{hint}
...
\begin{hint}
For an algebraic approach, use $z_1+z_2 = -z_3$ and apply the parallelogram law relating $|z_1+z_2|^2$ and $|z_1-z_2|^2$.
For a geometric approach, where is the centroid of this triangle? Where is its circumcenter?
\end{hint}

\begin{solution}
\textbf{Method 1 (Algebraic - Parallelogram Law):}
Since $z_1 + z_2 + z_3 = 0$, we have $z_1 + z_2 = -z_3$. 
Squaring the modulus of both sides:
...
Since all squared side lengths are equal to $3r^2$, the triangle is equilateral.

\textbf{Method 2 (Geometric):}
The centroid of the triangle is given by $\frac{z_1+z_2+z_3}{3} = 0$, which is the origin.
The circumcenter is also the origin, since all vertices lie on the circle $|z|=r$.
In any triangle, if the centroid and the circumcenter coincide, the triangle must be equilateral.
\end{solution}
...
\textbf{(ii)} To minimize $S(t)$, we can use calculus or algebra.

\textbf{Method 1 (Calculus):}
Find $S'(t) = 4t - 10$.
Set $4t - 10 = 0 \implies t = 2.5$.
Substitute $t=2.5$ back into $S(t)$:
\[ S(2.5) = 2(6.25) - 25 + 17 = 12.5 - 25 + 17 = 4.5 \]

\textbf{Method 2 (Algebra - Completing the Square):}
Rewrite $S(t)$ by completing the square:
\begin{align*}
    S(t) &= 2(t^2 - 5t) + 17 \\
    &= 2\left(t^2 - 5t + 6.25\right) - 2(6.25) + 17 \\
    &= 2(t - 2.5)^2 - 12.5 + 17 \\
    &= 2(t - 2.5)^2 + 4.5
\end{align*}
This instantly yields the vertex at $t = 2.5$ and the minimum squared distance of $4.5$.

In both cases, the minimum distance is $\sqrt{4.5} = \sqrt{\frac{9}{2}} = \frac{3}{\sqrt{2}} = \frac{3\sqrt{2}}{2}$.
\end{solution}
...
\begin{solution}
Let $e_1, e_2, e_3$ be the elementary symmetric polynomials. Then $z_1, z_2, z_3$ are the roots of 
\[ P(z) = z^3 - e_1 z^2 + e_2 z - e_3. \]

\textbf{Step 1: Find $e_1$.}
From the first condition, $e_1 = z_1 + z_2 + z_3 = 3$.

\textbf{Step 2: Find $e_2$.}
Use the identity $\sum z^2 = (\sum z)^2 - 2\sum z_1 z_2$:
\[ 3 = 3^2 - 2e_2 = 9 - 2e_2 \]
\[ 2e_2 = 6 \implies e_2 = 3 \]

\textbf{Step 3: Find $e_3$.}
By Newton's Sums, $S_3 - e_1 S_2 + e_2 S_1 - 3e_3 = 0$, where $S_k = \sum z_i^k$:
\[ 3 - 3 \cdot 3 + 3 \cdot 3 - 3e_3 = 0 \]
\[ 3 - 9 + 9 - 3e_3 = 0 \implies 3e_3 = 3 \implies e_3 = 1 \]
...
\begin{enumerate}
    ...
    \item Similar techniques can be used to show $\frac{1}{e} = e^{-1} = \sum_{n=0}^{\infty} \frac{(-1)^n}{n!}$ is irrational as well. The product of $e$ and $e^{-1}$ is 1, so if one were rational, the other would be rational too.
    \item Known fact:$e^r$ is irrational for any non-zero rational number $r$, published by Johann Heinrich Lambert in 1768 using continued fractions.
\end{enumerate}
\end{takeaways}
